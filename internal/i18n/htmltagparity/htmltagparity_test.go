...
func TestMismatchFormattingAndKnownTags(t *testing.T) {
	tests := []struct {
		...
		{
			name: "lowercase dot tags are ignored as markup (likely paths)",
			src:  "Hello <my.component>world</my.component>",
			tgt:  "Bonjour world",
			want: false,
		},
		{
			name: "lowercase underscore tags are ignored as markup (likely paths)",
			src:  "Hello <my_tag>world</my_tag>",
			tgt:  "Bonjour world",
			want: false,
		},
		{
			name: "lowercase tags with numbers are ignored as markup (likely paths)",
