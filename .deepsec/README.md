...
Currently configured project: `hyperlocalise` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/hyperlocalise/SETUP.md` to fill in
   `data/hyperlocalise/INFO.md`.
...
`--project-id hyperlocalise` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/hyperlocalise/`.
