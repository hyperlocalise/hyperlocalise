# Agent setup for `hyperlocalise`

This is a deepsec scanning workspace. Project `hyperlocalise` was just registered
(target: `..`). Setup is incomplete — `data/hyperlocalise/INFO.md`
...
2. **Fill in `data/hyperlocalise/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id hyperlocalise
pnpm deepsec process --project-id hyperlocalise
```
