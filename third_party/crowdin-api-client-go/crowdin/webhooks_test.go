...
func TestWebhooksService_Get(t *testing.T) {
	...
	const path = "/api/v2/projects/1/webhooks/2"
	...
	webhook, resp, err := client.Webhooks.Get(context.Background(), 1, 2)
...
func TestWebhooksService_List(t *testing.T) {
	...
	for projectID, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			path := fmt.Sprintf("/api/v2/projects/%d/webhooks", projectID)
			mux.HandleFunc(path, func(w http.ResponseWriter, r *http.Request) {
				testMethod(t, r, "GET")
			...
			webhooks, resp, err := client.Webhooks.List(context.Background(), projectID, tt.opts)
			require.NoError(t, err)
...
func TestWebhooksService_Add_requiredFields(t *testing.T) {
	...
	mux.HandleFunc(path, func(w http.ResponseWriter, r *http.Request) {
		testMethod(t, r, "POST")
		testURL(t, r, path)
		testBody(t, r, `{"name":"Proofread","url":"https://webhook.site/1c20d9b5-6e6a-4522-974d-9da7ea7595c9","events":["file.approved","file.translated"],"requestType":"POST"}`+"\n")
...
func TestWebhooksService_Edit(t *testing.T) {
	...
	for whID, tt := range tests {
		path := fmt.Sprintf("/api/v2/projects/1/webhooks/%d", whID)
		mux.HandleFunc(path, func(w http.ResponseWriter, r *http.Request) {
		...
		webhook, resp, err := client.Webhooks.Edit(context.Background(), 1, whID, tt.req)
		if tt.err == "" {
			require.NoError(t, err)
			assert.NotNil(t, resp)
			assert.Equal(t, 4, webhook.ID)
...
func TestWebhooksService_Delete(t *testing.T) {
	...
	resp, err := client.Webhooks.Delete(context.Background(), 1, 2)
	require.NoError(t, err)
