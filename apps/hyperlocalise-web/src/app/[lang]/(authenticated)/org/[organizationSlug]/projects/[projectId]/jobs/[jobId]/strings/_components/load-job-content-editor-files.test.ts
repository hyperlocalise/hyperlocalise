...
import {
  ...
  resolveJobContentEditorTargetFromStoredFileId,
...
describe("resolveJobContentEditorTargetFromStoredFileId", () => {
  it("returns the matching file when present", () => {
    const files = [createFile({ storedFileId: "file_en", sourcePath: "locales/en.json" })];

    expect(resolveJobContentEditorTargetFromStoredFileId(files, "file_en")).toEqual({
      status: "found",
      file: files[0],
    });
  });
  ...
  it("returns not_found when the file is absent and the list is below the fetch limit", () => {
    expect(resolveJobContentEditorTargetFromStoredFileId([createFile()], "file_missing")).toEqual({
      status: "not_found",
  ...
  it("returns list_truncated when the file is absent and the list hit the fetch limit", () => {
    ...
    expect(resolveJobContentEditorTargetFromStoredFileId(files, "file_missing")).toEqual({
