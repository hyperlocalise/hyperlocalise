...
export type JobContentEditorDefaultFileReference = {
...
function resolveTargetLocale(file: ProjectFileRecord, requestedTargetLocale: string | null) {
  if (file.provider) {
    return selectJobContentEditorTargetLocale({
...
export function resolveDefaultJobContentEditorFileReference(
  files: ProjectFileRecord[],
  requestedTargetLocale: string | null,
): JobContentEditorDefaultFileReference | null {
  for (const file of sortFilesByPath(files)) {
    ...
    if (supportsProviderContentEditorFile(file)) {
      return {
