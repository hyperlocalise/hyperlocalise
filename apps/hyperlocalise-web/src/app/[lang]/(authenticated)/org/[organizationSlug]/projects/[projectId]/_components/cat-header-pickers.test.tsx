...
import { CatFileTreePicker, catSourcePathDisplayName } from "./cat-header-pickers";
...
describe("catSourcePathDisplayName", () => {
  it("returns the basename of a nested source path", () => {
    expect(catSourcePathDisplayName("lang/en-US.json")).toBe("en-US.json");
  });

  it("returns the original path when it has no directories", () => {
    expect(catSourcePathDisplayName("en-US.json")).toBe("en-US.json");
  });
});

describe("CatFileTreePicker repository selection", () => {
  ...
  it("commits the chosen repository under All Files before navigating", async () => {
    ...
    expect(onRepositoryChange).toHaveBeenCalledWith("acme/docs", CAT_ALL_FILES_SOURCE_PATH);
