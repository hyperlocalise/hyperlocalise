...
import { resolveJobContentEditorSelectableTargetLocales } from "./job-content-editor-target-locale";

export const PROJECT_FILES_FETCH_LIMIT = 1_000;

export type JobContentEditorTargetFileLoadResult =
...
export function resolveJobContentEditorTargetFromStoredFileId(
  ...
  storedFileId: string,
): JobContentEditorTargetFileLoadResult {
...
export async function loadJobContentEditorTargetFile(input: {
...
}): Promise<JobContentEditorTargetFileLoadResult> {
  if (input.sourcePath) {
  ...
  if (input.storedFileId) {
    ...
    return resolveJobContentEditorTargetFromStoredFileId(files, input.storedFileId);
  }
...
export async function loadJobContentEditorSelectableTargetLocales(input: {
  organizationSlug: string;
  projectId: string;
  jobId: string;
}) {
  ...
  if (parsedJobId) {
    ...
    return resolveJobContentEditorSelectableTargetLocales({
  ...
  return resolveJobContentEditorSelectableTargetLocales(job);
}

export async function loadJobContentEditorProviderJobFiles(input: {
  organizationSlug: string;
  projectId: string;
  jobId: string;
  targetLocale?: string | null;
...
export async function loadJobContentEditorJobSourceFiles(input: {
  organizationSlug: string;
  projectId: string;
  jobId: string;
  targetLocale?: string | null;
  ...
  if (parseProviderJobId(input.jobId)) {
    return loadJobContentEditorProviderJobFiles(input);
  }
  ...
  const providerFiles = await loadJobContentEditorProviderJobFiles({ ...input, job });
