...
type StartupsRouteProps = {
  params: Promise<{ lang: string }>;
};

export async function generateMetadata({ params }: StartupsRouteProps): Promise<Metadata> {
...
export default async function StartupsRoutePage({ params }: StartupsRouteProps) {
  const { lang } = await params;
  const locale = normalizeAppLocale(lang) ?? DEFAULT_APP_LOCALE;

  return <StartupsPage locale={locale} />;
}
