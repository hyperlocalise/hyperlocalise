...
export const GITLAB_STATE_TTL_MS = 60 * 60 * 1000;
...
export async function signGitlabState(payload: string, secret: string): Promise<string> {
...
export async function createGitlabState(
  slug: string,
  secret: string,
  nonce: string,
  timestamp = Date.now(),
  ...
  return `${payload}:${await signGitlabState(payload, secret)}`;
}

export async function verifyGitlabState(
  state: string,
  secret: string,
  ...
  const expectedSignature = await signGitlabState(payload, secret);
  ...
  if (Date.now() - timestamp > GITLAB_STATE_TTL_MS) return null;
...
export function getGitlabStateSecret(): string {
  const secret = env.GITLAB_OAUTH_STATE_SECRET;
  if (!secret) {
    throw new Error("missing GITLAB_OAUTH_STATE_SECRET");
  }
  return secret;
}
