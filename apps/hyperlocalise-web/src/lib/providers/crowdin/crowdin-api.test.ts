...
describe("CrowdinApiClient", () => {
  ...
  it("sends webhook secrets through Crowdin secret field, not custom headers", async () => {
      ...
      return new Response(
        JSON.stringify({
          data: {
            ...
            events: ["file.updated"],
            headers: { "X-Provider-Webhook-Id": "pending" },
            isActive: true,
          },
        }),
        { status: 200 },
    ...
    const client = createClient(fetchMock);
    await client.createProjectWebhook(42, {
      name: "Hyperlocalise",
      url: "https://app.example.test/api/webhooks/tms/crowdin",
      events: ["file.updated"],
    ...
    expect(body.secret).toBe("webhook-signing-secret");
    expect(body.headers).toEqual({ "X-Provider-Webhook-Id": "pending" });
  });

  it("patches webhook secrets through Crowdin secret field", async () => {
      ...
      return new Response(
        JSON.stringify({
          data: {
            ...
            url: "https://app.example.test/api/webhooks/tms/crowdin",
            events: ["file.updated"],
            headers: { "X-Provider-Webhook-Id": "7" },
            isActive: true,
          },
        }),
        { status: 200 },
    ...
    await client.updateProjectWebhook(42, 7, {
      secret: "webhook-signing-secret",
      headers: { "X-Provider-Webhook-Id": "7" },
    });
