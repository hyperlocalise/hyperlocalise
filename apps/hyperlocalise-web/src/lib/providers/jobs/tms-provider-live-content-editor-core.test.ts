...
async function setupCrowdinPatCredential(input: {
  ...
  const connectionResult = await crowdinAuth.upsertUserPatConnection({
    organizationId: input.organizationId,
    userId: input.userId,
...
describe("getTmsProviderLiveCatFile", () => {
  ...
  it("loads Crowdin queue segments with translations without approval or comment API calls", async () => {
    ...
    const contentEditorFile = await getTmsProviderLiveCatFile(
      organization.id,
      "42",
      "home.json",
      "fr",
    ...
    expect(contentEditorFile).toMatchObject({
      sourcePath: "home.json",
      targetLocale: "fr",
      canEditTranslations: true,
      truncated: false,
    });
    expect(contentEditorFile?.provider?.targetLocales).toEqual(["fr"]);
    expect(contentEditorFile?.segments).toHaveLength(2);
    expect(contentEditorFile?.segments[0]).toMatchObject({
      externalStringId: "1001",
      key: "hero.title",
      sourceText: "Hello",
      isHidden: true,
    });
    expect(contentEditorFile?.segments[0]).not.toHaveProperty("target");
    expect(contentEditorFile?.segments[1]).toMatchObject({
      externalStringId: "1002",
      sourceText: JSON.stringify({ one: "Start", other: "Start all" }),
    });
    expect(contentEditorFile?.segments[1]).not.toHaveProperty("isHidden");
    expect(contentEditorFile?.segments[1]).not.toHaveProperty("target");
  ...
  it("loads a Crowdin queue directly by provider resource id", async () => {
    ...
    expect(contentEditorFile?.segments).toHaveLength(1);
    expect(contentEditorFile?.segments[0]).toMatchObject({
      externalStringId: "1001",
      key: "hero.title",
      sourceText: "Hello",
  ...
  it("loads paginated Crowdin queue pages without walking the full file for totals", async () => {
    ...
    expect(contentEditorFile?.pagination).toMatchObject({
      offset: 0,
      limit: 50,
      returnedCount: 50,
      totalCount: 51,
  ...
  it("walks Crowdin status bands for untranslated-first sort", async () => {
    ...
    expect(contentEditorFile?.segments.map((segment) => segment.externalStringId)).toEqual([
      "1001",
      "1002",
    ]);
    expect(contentEditorFile?.pagination).toMatchObject({
  ...
  it("marks every visible has_issues segment with unresolved issue flags without per-string comment calls", async () => {
    ...
    expect(contentEditorFile?.segments).toHaveLength(2);
    expect(
      contentEditorFile?.segments.every((segment) => !("unresolvedIssueCount" in segment)),
    ).toBe(true);
...
describe("getTmsProviderLiveContentEditorAllFiles", () => {
  let originalFetch: typeof fetch;
  ...
  it("scopes small All Files queues with a file CROQL so pagination stays accurate", async () => {
    ...
    const contentEditorFile = await getTmsProviderLiveContentEditorAllFiles(
      organization.id,
      "42",
      "fr",
      {
    ...
    expect(contentEditorFile.segments).toEqual([
  ...
  it("rejects narrowed sourcePaths when the file CROQL would exceed the soft cap", async () => {
    ...
    await expect(
      getTmsProviderLiveContentEditorAllFiles(organization.id, "42", "fr", {
        actorUserId: user.id,
        sourcePaths,
        pagination: {
          offset: 0,
