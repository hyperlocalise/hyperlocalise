...
function tokenBundle(overrides: Partial<LokaliseOAuthTokenBundle> = {}): LokaliseOAuthTokenBundle {
  return {
    clientId: "client-id",
...
async function createLokaliseOAuthCredential() {
...
describe("lokalise user connections", () => {
  beforeAll(async () => {
  ...
  it("persists a user-level Lokalise OAuth connection summary", async () => {
    vi.useFakeTimers();
    ...
    if (isErr(result)) {
      throw new Error("expected Lokalise user connection upsert to succeed");
    }
    expect(result.value).toMatchObject({
      lokaliseUserId: 12345,
      username: "lokalise-user",
      email: "lokalise-user@example.com",
      fullName: "Lokalise User",
  ...
  it("rejects linking the same Lokalise user to another organization member", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-01-01T00:00:00.000Z"));
    ...
    if (!isErr(duplicateResult)) {
      throw new Error("expected duplicate Lokalise user link to fail");
    }
    expect(duplicateResult.error).toEqual({ code: "lokalise_user_already_linked" });
  });

  it("rejects duplicate Lokalise user links via DB unique constraint when pre-check races", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-01-01T00:00:00.000Z"));
    const { authContext, credential, identity } = await createLokaliseOAuthCredential();
  ...
  it("refreshes expired user access tokens and persists the new token bundle", async () => {
    ...
    const [updatedConnection] = await db
      ...
      .where(eq(schema.lokaliseUserConnections.id, connection!.id))
      .limit(1);
    expect(updatedConnection!.oauthExpiresAt?.toISOString()).toBe("2026-01-01T02:00:00.000Z");
    expect(decryptLokaliseOAuthTokenBundle(updatedConnection!)).toMatchObject({
  ...
  it("propagates Lokalise refresh failures for expired user tokens", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-01-01T00:00:00.000Z"));
    const { authContext, credential } = await createLokaliseOAuthCredential();
    const upsertResult = await upsertLokaliseUserConnection({
    ...
    ).rejects.toThrow("lokalise_oauth_refresh_failed");
    expect(fetchMock).toHaveBeenCalledOnce();
  });
});
