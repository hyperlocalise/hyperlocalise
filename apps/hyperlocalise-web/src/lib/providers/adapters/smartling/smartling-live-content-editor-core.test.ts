...
describe("smartling-live-content-editor", () => {
  ...
  it("loads file segments without embedding targets in the queue", async () => {
    ...
    const contentEditorFile = await smartlingTmsProvider.buildLiveCatFile({
    ...
    expect(contentEditorFile.canEditTranslations).toBe(true);
  });
