...
import { resolveJobContentEditorInitialQueueFilter } from "./resolve-job-content-editor-initial-queue-filter";
...
describe("resolveJobContentEditorInitialQueueFilter", () => {
  beforeEach(() => {
  ...
  it("returns the URL queue filter when present", async () => {
    await expect(
      resolveJobContentEditorInitialQueueFilter({
        auth,
        jobId: "job_1",
