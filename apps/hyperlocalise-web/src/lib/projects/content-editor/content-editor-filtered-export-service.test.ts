...
vi.mock("./content-editor-filtered-export", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./content-editor-filtered-export")>();
  return {
    ...actual,
...
import {
  ...
  type ContentEditorQueueLoader,
} from "./content-editor-filtered-export-service";
...
const baseQuery: ProjectFileContentEditorQuery = {
...
describe("collectCatFilteredExportRows", () => {
  ...
  it("returns empty when the filtered queue has no segments", async () => {
    const loadCatQueue = vi.fn<ContentEditorQueueLoader>().mockResolvedValue({
      kind: "ok",
      contentEditorQueue: queuePage({ segments: [], hasMore: false }),
    });
