...
export function serializeXlsx(document: GlossaryInterchangeDocument): SerializationResult {
  const concepts = document.concepts.map((concept) => ({
    ...
    externalKey: asString(concept.externalKey),
    externalUserId: asString(concept.externalUserId),
    externalCreatedAt: asString(concept.externalCreatedAt),
    externalUpdatedAt: asString(concept.externalUpdatedAt),
...
export function parseXlsx(content: Uint8Array): GlossaryImportDocument {
  ...
  for (const [index, row] of conceptRows.entries()) {
    ...
    concepts.set(id, {
      ...
      externalKey: optionalCell(row, "externalKey"),
      externalUserId: optionalCell(row, "externalUserId"),
      externalCreatedAt: optionalCell(row, "externalCreatedAt"),
      externalUpdatedAt: optionalCell(row, "externalUpdatedAt"),
