    ...
    if (frame.local === "term" && currentTerm) currentTerm.term = value;
      ...
      if (!currentTerm.term || !currentTerm.locale) {
        ...
        const term = {
          ...
          gender: currentTerm.gender ?? null,
          termType: currentTerm.termType ?? null,
          url: currentTerm.url ?? null,
          lemma: currentTerm.lemma ?? null,
          status: currentTerm.status ?? "draft",
          caseSensitive: currentTerm.caseSensitive ?? false,
          forbidden: currentTerm.forbidden ?? false,
          provenance: currentTerm.provenance ?? "manual",
          reviewStatus: currentTerm.reviewStatus ?? "approved",
          metadata: currentTerm.metadata ?? {},
          createdAt: currentTerm.createdAt ?? new Date(0).toISOString(),
          updatedAt: currentTerm.updatedAt ?? new Date(0).toISOString(),
        } satisfies GlossaryInterchangeTerm;
