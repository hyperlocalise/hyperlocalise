...
/**
 * Stores short-lived Crowdin OAuth state for linking the signed-in Hyperlocalise user to
 * their own Crowdin account. This is separate from the organization-level integration
 * setup state so account linking can be offered to every workspace member.
 */
export const crowdinUserOAuthStates = pgTable(
  "crowdin_user_oauth_states",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    nonce: text("nonce").notNull(),
  ...
  (table) => [
    uniqueIndex("crowdin_user_oauth_states_nonce_key").on(table.nonce),
    index("idx_crowdin_user_oauth_states_org_user").on(table.organizationId, table.userId),
    index("idx_crowdin_user_oauth_states_expires_at").on(table.expiresAt),
  ],
);

/**
 * Links a Hyperlocalise user to their Crowdin identity and stores the user's OAuth
 * token bundle for user-attributed Crowdin actions.
 */
export const crowdinUserConnections = pgTable(
  "crowdin_user_connections",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    organizationId: uuid("organization_id")
      .notNull()
  ...
  (table) => [
    uniqueIndex("crowdin_user_connections_org_user_key").on(table.organizationId, table.userId),
    uniqueIndex("crowdin_user_connections_org_crowdin_user_key").on(
      table.organizationId,
      table.crowdinUserId,
    ),
    index("idx_crowdin_user_connections_provider_credential").on(table.providerCredentialId),
  ],
);

/**
 * Stores short-lived Phrase OAuth state for linking the signed-in Hyperlocalise user to
 * their own Phrase account. This mirrors Crowdin's user-link flow while keeping provider
 * tokens separate.
 */
export const phraseUserOAuthStates = pgTable(
  "phrase_user_oauth_states",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    nonce: text("nonce").notNull(),
    codeVerifier: text("code_verifier").notNull(),
  ...
  (table) => [
    uniqueIndex("phrase_user_oauth_states_nonce_key").on(table.nonce),
    index("idx_phrase_user_oauth_states_org_user").on(table.organizationId, table.userId),
    index("idx_phrase_user_oauth_states_expires_at").on(table.expiresAt),
  ],
);

/**
 * Links a Hyperlocalise user to their Phrase identity and stores the user's OAuth token
 * bundle for user-attributed Phrase actions.
 */
export const phraseUserConnections = pgTable(
  "phrase_user_connections",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    organizationId: uuid("organization_id")
      .notNull()
    ...
    providerCredentialId: uuid("provider_credential_id")
      .notNull()
      .references(() => organizationExternalTmsProviderCredentials.id, { onDelete: "cascade" }),
    phraseUserUid: text("phrase_user_uid").notNull(),
    username: text("username").notNull(),
    email: text("email"),
    fullName: text("full_name"),
    oauthExpiresAt: timestamp("oauth_expires_at", { withTimezone: true }),
  ...
  (table) => [
    uniqueIndex("phrase_user_connections_org_user_key").on(table.organizationId, table.userId),
    uniqueIndex("phrase_user_connections_org_phrase_user_key").on(
      table.organizationId,
      table.phraseUserUid,
    ),
    index("idx_phrase_user_connections_provider_credential").on(table.providerCredentialId),
  ],
);

/**
 * Stores short-lived Lokalise OAuth state for linking the signed-in Hyperlocalise
 * user to their own Lokalise account.
 */
...
/**
 * Links a Hyperlocalise user to Lokalise and stores the user's OAuth token bundle
 * for user-attributed Lokalise actions.
 */
