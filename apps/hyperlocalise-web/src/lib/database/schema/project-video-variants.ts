...
/**
 * Stores per-locale localized video variants for file-backed MP4 sources.
 */
export const projectVideoVariants = pgTable(
  "project_video_variants",
  {
  ...
  (table) => [
    uniqueIndex("project_video_variants_project_path_locale").on(
      table.projectId,
      table.sourcePath,
      table.targetLocale,
    ),
    index("idx_project_video_variants_org_project").on(table.organizationId, table.projectId),
    index("idx_project_video_variants_repo_file").on(table.repositorySourceFileId),
    index("idx_project_video_variants_external_file").on(table.externalTmsFileId),
    index("idx_project_video_variants_status").on(table.projectId, table.status),
  ],
);
