...
/**
 * Stores one project-level markdown guideline for localization guidance.
 */
export const projectKnowledgeMemories = pgTable(
  "project_knowledge_memories",
  {
  ...
  (table) => [
    uniqueIndex("project_knowledge_memories_revision_id_key").on(table.revisionId),
    check(
      "project_knowledge_memories_content_length_check",
      sql`char_length(${table.content}) <= 50000`,
    ),
    check(
      "project_knowledge_memories_summary_length_check",
      sql`char_length(${table.summary}) <= 160`,
    ),
    check("project_knowledge_memories_version_check", sql`${table.version} >= 1`),
  ],
);

/**
 * Stores immutable snapshots after a project guideline revision is superseded.
 */
export const projectKnowledgeMemoryRevisions = pgTable(
  "project_knowledge_memory_revisions",
  {
    id: uuid("id").primaryKey(),
    projectId: text("project_id")
      .notNull()
  ...
  (table) => [
    uniqueIndex("project_knowledge_memory_revisions_project_version_key").on(
      table.projectId,
      table.version,
    ),
    check(
      "project_knowledge_memory_revisions_content_length_check",
      sql`char_length(${table.content}) <= 50000`,
    ),
    check(
      "project_knowledge_memory_revisions_summary_length_check",
      sql`char_length(${table.summary}) <= 160`,
    ),
