...
import { loadContentEditorSegmentConcordance } from "./content-editor-core";
...
describe("loadContentEditorSegmentConcordance", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("resolves per-user Crowdin credentials for live concordance", async () => {
    await loadContentEditorSegmentConcordance({
      organizationId: "org_1",
  ...
  it("maps provider concept matches into CAT glossaryConcepts", async () => {
    ...
    const result = await loadContentEditorSegmentConcordance({
      organizationId: "org_1",
