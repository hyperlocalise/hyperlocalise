...
import type {
  ...
  ProjectFileContentEditorRecommendationResponse,
  ProjectFileContentEditorResponse,
...
const {
  ...
  loadContentEditorSegmentConcordanceMock,
  loadContentEditorSegmentVisualContextMock,
  generateContentEditorAiRecommendationMock,
  ...
  loadContentEditorSegmentConcordanceMock: vi.fn(),
  loadContentEditorSegmentVisualContextMock: vi.fn(),
  generateContentEditorAiRecommendationMock: vi.fn(),
...
vi.mock("@/lib/flags/release-flags", () => ({
  ...
  RELEASE_CAT_ALL_FILES_FLAG: "release-content-editor-all-files",
}));

vi.mock("@/lib/translation/content-editor-core", () => ({
  loadContentEditorSegmentConcordance: (...args: unknown[]) =>
    loadContentEditorSegmentConcordanceMock(...args),
  loadContentEditorSegmentVisualContext: (...args: unknown[]) =>
    loadContentEditorSegmentVisualContextMock(...args),
  generateContentEditorAiRecommendation: (...args: unknown[]) =>
    generateContentEditorAiRecommendationMock(...args),
}));
...
vi.mock("@/lib/providers/jobs/tms-provider-live", async (importOriginal) => {
  ...
  return {
    ...
    getTmsProviderLiveContentEditorAllFiles: (...args: unknown[]) =>
      getTmsProviderLiveContentEditorAllFilesMock(...args),
...
afterEach(async () => {
  vi.clearAllMocks();
  isReleaseContentEditorAllFilesEnabledMock.mockResolvedValue(false);
...
describe("project file CAT routes", () => {
  it("rejects All Files CAT when the release flag is off", async () => {
    ...
    expect(getTmsProviderLiveContentEditorAllFilesMock).not.toHaveBeenCalled();
  });
  ...
  it("loads Crowdin All Files CAT when the release flag is on", async () => {
    ...
    isReleaseContentEditorAllFilesEnabledMock.mockResolvedValue(true);
    getTmsProviderLiveContentEditorAllFilesMock.mockResolvedValue({
      sourcePath: "*",
      filename: "All Files",
      provider: {
        kind: "crowdin",
    ...
    expect(getTmsProviderLiveContentEditorAllFilesMock).toHaveBeenCalledWith(
      expect.any(String),
      "42",
      "fr",
    ...
    expect(isReleaseContentEditorAllFilesEnabledMock).toHaveBeenCalledWith("crowdin");
  });
  ...
  it("rejects All Files CAT when decide disables the provider", async () => {
    ...
    expect(response.status).toBe(403);
    expect(await response.json()).toMatchObject({ error: "feature_unavailable" });
    expect(isReleaseContentEditorAllFilesEnabledMock).toHaveBeenCalledWith("phrase");
    expect(getTmsProviderLiveContentEditorAllFilesMock).not.toHaveBeenCalled();
  });
  ...
  it("rejects All Files CAT at the TMS layer when Flags Explorer overrides the flag on for an unsupported provider", async () => {
    ...
    isReleaseContentEditorAllFilesEnabledMock.mockResolvedValue(true);
    getTmsProviderLiveContentEditorAllFilesMock.mockRejectedValue(
    ...
    expect(isReleaseContentEditorAllFilesEnabledMock).toHaveBeenCalledWith("phrase");
    expect(getTmsProviderLiveContentEditorAllFilesMock).toHaveBeenCalled();
  });
  ...
  it("returns Crowdin AI recommendations for an encoded provider project", async () => {
    ...
    generateContentEditorAiRecommendationMock.mockResolvedValue(
      ok({
    ...
    expect(response.status).toBe(200);
    const body = (await response.json()) as ProjectFileContentEditorRecommendationResponse;
    ...
    expect(generateContentEditorAiRecommendationMock).toHaveBeenCalledWith(
      expect.objectContaining({
        projectId: "ext:crowdin:42",
        sourcePath: "crowdin/home.json",
        key: "hello",
  ...
  it("returns Crowdin concordance matches for a CAT segment", async () => {
    ...
    loadContentEditorSegmentConcordanceMock.mockResolvedValue({
      glossaryTerms: [
        {
          id: "glossary-1",
          source: "workspace",
    ...
    expect(response.status).toBe(200);
    const body = (await response.json()) as ProjectFileContentEditorConcordanceResponse;
    expect(body.concordance.glossaryTerms).toHaveLength(1);
    expect(body.concordance.translationMemoryMatches).toHaveLength(1);
    expect(loadContentEditorSegmentConcordanceMock).toHaveBeenCalledWith(
      expect.objectContaining({
        projectId: "ext:crowdin:42",
        providerKind: "crowdin",
  ...
  it("loads provider visual context for TMS segments", async () => {
    ...
    loadContentEditorSegmentVisualContextMock.mockResolvedValue({
      screenshots: [
        {
          id: "12",
          name: "Checkout",
    ...
    expect(loadContentEditorSegmentVisualContextMock).toHaveBeenCalledWith(
      expect.objectContaining({
        providerKind: "crowdin",
        externalProjectId: "42",
        externalStringId: "99",
  ...
  it("returns native CAT comment counts on list and comments on segment detail", async () => {
    ...
    expect(response.status).toBe(200);
    const body = (await response.json()) as ProjectFileContentEditorResponse;
    expect(body.contentEditorFile.segments[0]).not.toHaveProperty("comments");
    ...
    const targetBody =
      (await targetResponse.json()) as ProjectFileContentEditorSegmentTargetResponse;
    ...
    const commentsBody =
      (await commentsResponse.json()) as ProjectFileContentEditorSegmentCommentsResponse;
  ...
  it("loads native segment targets with All Files sourcePath=*", async () => {
    ...
    const allFilesTargetBody =
      (await allFilesTargetResponse.json()) as ProjectFileContentEditorSegmentTargetResponse;
  ...
  it("toggles treat-as-image metadata on a native CAT segment", async () => {
    ...
    const contentEditorResponse = await client.api.orgs[":organizationSlug"].projects[
      ":projectId"
    ].files.detail.cat.$get(
      {
        param: {
    ...
    expect(contentEditorResponse.status).toBe(200);
    const contentEditorBody =
      (await contentEditorResponse.json()) as ProjectFileContentEditorResponse;
    expect(contentEditorBody.contentEditorFile.segments[0]).toMatchObject({
      contentKind: "image_url",
      looksLikeImageUrl: true,
    });
  });
  ...
  it("returns Crowdin CAT content for an encoded provider project", async () => {
    ...
    expect(response.status).toBe(200);
    const body = (await response.json()) as ProjectFileContentEditorResponse;
    expect(body.contentEditorFile.segments[0]).toMatchObject({ externalStringId: "1001" });
    expect(body.contentEditorFile.segments[0]).not.toHaveProperty("target");
    expect(body.contentEditorFile.segments[0]).not.toHaveProperty("comments");
  ...
  it("returns CAT queue content from the split queue endpoint", async () => {
    ...
    expect(response.status).toBe(200);
    const body = (await response.json()) as ProjectFileContentEditorQueueResponse & {
      contentEditorFile?: unknown;
    };
    expect(body.contentEditorFile).toBeUndefined();
    expect(body.contentEditorQueue).toMatchObject({
      sourcePath: "crowdin/home.json",
      canContributeTeamGlossary: false,
      pagination: {
        offset: 25,
  ...
  it("passes pagination params to the provider CAT loader", async () => {
    ...
    expect(response.status).toBe(200);
    const body = (await response.json()) as ProjectFileContentEditorResponse;
    expect(body.contentEditorFile.pagination).toMatchObject({
      offset: 50,
      limit: 25,
      hasMore: true,
      totalCount: 120,
  ...
  it("saves Crowdin CAT translations for users with write-back permission", async () => {
    ...
    expect(response.status).toBe(200);
    const body = (await response.json()) as ProjectFileContentEditorTranslationResponse;
  ...
  it("posts Crowdin CAT comments for users with write-back permission", async () => {
    ...
    expect(response.status).toBe(200);
    const body = (await response.json()) as ProjectFileContentEditorCommentResponse;
  ...
  it("hides and unhides native CAT source strings in bulk", async () => {
    ...
    const hiddenBody = (await hiddenQueue.json()) as ProjectFileContentEditorQueueResponse;
    expect(hiddenBody.contentEditorQueue.canContributeTeamGlossary).toBe(true);
    expect(hiddenBody.contentEditorQueue.segments).toHaveLength(2);
    expect(
      hiddenBody.contentEditorQueue.segments.every((segment) => segment.isHidden === true),
    ).toBe(true);
    ...
    const remainingBody = (await remainingHidden.json()) as ProjectFileContentEditorQueueResponse;
    expect(
  ...
  it("locks and unlocks native CAT segments and shows isLocked on the queue", async () => {
    ...
    expect(await lockResponse.json()).toEqual({
      contentEditorSegmentLock: { updatedCount: 2, isLocked: true },
    });
    ...
    const lockedBody = (await lockedQueue.json()) as ProjectFileContentEditorQueueResponse;
    expect(
      lockedBody.contentEditorQueue.segments.every((segment) => segment.isLocked === true),
    ).toBe(true);
    ...
    expect(await unlockResponse.json()).toEqual({
      contentEditorSegmentLock: { updatedCount: 1, isLocked: false },
    });
    ...
    const remainingBody = (await remainingQueue.json()) as ProjectFileContentEditorQueueResponse;
    const greetingSegment = remainingBody.contentEditorQueue.segments.find(
      (segment) => segment.externalStringId === greeting!.id,
    );
    const farewellSegment = remainingBody.contentEditorQueue.segments.find(
  ...
  it("locks Crowdin CAT segments without calling the provider hide API", async () => {
    ...
    expect(lockResponse.status).toBe(200);
    expect(await lockResponse.json()).toEqual({
      contentEditorSegmentLock: { updatedCount: 1, isLocked: true },
    });
    expect(setTmsProviderLiveCatStringsHiddenMock).not.toHaveBeenCalled();
    ...
    const queueBody = (await queueResponse.json()) as ProjectFileContentEditorQueueResponse;
    expect(queueBody.contentEditorQueue.segments[0]?.isLocked).toBe(true);
  ...
  it("returns all organization teams in CAT contributorTeams for operators", async () => {
    ...
    const adminBody = (await adminQueueResponse.json()) as ProjectFileContentEditorQueueResponse;
    expect(adminBody.contentEditorQueue.contributorTeams?.map((team) => team.name)).toEqual(
      expect.arrayContaining(["CAT Alpha", "CAT Beta"]),
    );
    expect(adminBody.contentEditorQueue.contributorTeams?.map((team) => team.name)).not.toContain(
    ...
    const translatorBody =
      (await translatorQueueResponse.json()) as ProjectFileContentEditorQueueResponse;
    expect(translatorBody.contentEditorQueue.contributorTeams).toEqual([
