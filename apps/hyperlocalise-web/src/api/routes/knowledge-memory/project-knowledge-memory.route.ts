...
import {
  getKnowledgeMemoryRevisionForProject,
  listKnowledgeMemoryRevisionsForProject,
...
export function createProjectKnowledgeMemoryRoutes() {
  return new Hono<{ Variables: AuthVariables }>()
    ...
    .get("/", async (c) => {
      ...
      const knowledgeMemory = await getKnowledgeMemoryForProject(project.id);
    ...
    .get("/revisions", validateKnowledgeMemoryRevisionListQuery, async (c) => {
      ...
      const result = await listKnowledgeMemoryRevisionsForProject({
    ...
    .get("/revisions/:revisionId", validateKnowledgeMemoryRevisionParams, async (c) => {
      ...
      const result = await getKnowledgeMemoryRevisionForProject({
        projectId: project.id,
        revisionId,
      });
    ...
    .post("/revisions/:revisionId/restore", validateKnowledgeMemoryRevisionParams, async (c) => {
      ...
      const result = await restoreKnowledgeMemoryRevisionForProject({
        projectId: project.id,
        revisionId,
    ...
    .put("/", validateUpdateKnowledgeMemoryBody, async (c) => {
      ...
      const payload = c.req.valid("json");
      const result = await commitKnowledgeMemoryForProject({
        projectId: project.id,
