...
describe("cat-office-mime", () => {
  it.each([
    ...
    expect(catOfficeUploadAccept(viewerId)).toBe(accept);
  });

  it("returns undefined accept for image and unknown viewers", () => {
    expect(catOfficeUploadAccept("image")).toBeUndefined();
    expect(catOfficeUploadAccept(null)).toBeUndefined();
  });
