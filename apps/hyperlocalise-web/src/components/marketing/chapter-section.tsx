...
function TmsLogoMarquee() {
  return (
    <div className="mt-8 overflow-hidden">
      <div className="mb-3 text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-white/38">
        Works with your TMS
      </div>
          ...
          {[0, 1].map((track) => (
            <div key={track} className="flex shrink-0 items-center gap-8 pr-8">
              {tmsLogos.map((logo) => (
                <span
                  key={`${track}-${logo}`}
                  className="text-sm font-medium tracking-[0.14em] whitespace-nowrap uppercase text-white/58"
                >
                  {logo}
                </span>
              ))}
  ...
  return (
    <section>
      <div className="max-w-2xl">
        ...
        <TypographyP className="mt-5 max-w-xl text-white/60">{chapter.description}</TypographyP>
        {chapter.cta ? (
          <div className="mt-5">
            <Button
              variant="outline"
              className="rounded-full border-white/12 bg-white/3 px-4 text-white hover:bg-white/6"
              nativeButton={false}
        ...
        <div className="mt-6 text-sm text-white/45">
          {chapter.id}.0 {chapter.label} <span className="text-white/70">→</span>
        </div>
      </div>
      ...
      <div className="mt-8 grid gap-4 text-sm text-white/40 sm:grid-cols-2 lg:grid-cols-4">
