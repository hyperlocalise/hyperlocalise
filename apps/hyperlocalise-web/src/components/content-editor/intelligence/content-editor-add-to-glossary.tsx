...
export function ContentEditorAddToGlossary({
  organizationSlug,
  projectId,
  teamId,
  teamName,
  ...
  showTitle?: boolean;
  onAdded?: () => void;
  onTeamGlossaryCreated?: (glossary: ContentEditorTeamGlossaryOption) => void;
}) {
  const intl = useIntl();
  ...
  const [createdGlossaries, setCreatedGlossaries] = useState<ContentEditorTeamGlossaryOption[]>([]);
  ...
  const createGlossary = useMutation({
    mutationFn: async () => {
      if (!organizationSlug || !projectId) {
        throw new Error(
          intl.formatMessage(contentEditorIntelligencePanelMessages.addToGlossaryCreateFailed),
        );
      }
    ...
    onSuccess: (body) => {
      const created: ContentEditorTeamGlossaryOption = {
        id: body.glossary.id,
      ...
      toast.success(
        intl.formatMessage(contentEditorIntelligencePanelMessages.addToGlossaryCreateSuccess),
      );
    },
    onError: (error: Error) => {
  ...
  const addConcept = useMutation({
    mutationFn: async () => {
      if (!organizationSlug || !selectedGlossary) {
        throw new Error(
          intl.formatMessage(contentEditorIntelligencePanelMessages.addToGlossaryUnavailable),
        );
      }
      ...
      if (!response.ok) {
        throw new Error(
          await readApiError(
            response,
            intl.formatMessage(contentEditorIntelligencePanelMessages.addToGlossaryFailed),
          ),
        );
      }
      return response.json();
    ...
    onSuccess: () => {
      toast.success(
        intl.formatMessage(contentEditorIntelligencePanelMessages.addToGlossarySuccess),
      );
      onAdded?.();
    },
    onError: (error: Error) => {
  ...
  const glossaryName =
    ...
    intl.formatMessage(contentEditorIntelligencePanelMessages.addToGlossaryPickerPlaceholder);
    ...
    <div className="rounded-xl border border-border bg-muted/20 p-3">
      {showTitle ? (
        <h3 className="text-sm font-medium text-foreground">
          <FormattedMessage {...contentEditorIntelligencePanelMessages.addToGlossaryTitle} />
        </h3>
      ) : null}
      <div className={cn("grid gap-3", showTitle && "mt-3")}>
        ...
        <Field className="min-w-0 gap-1.5">
          <FieldLabel htmlFor={primaryTermId}>
            <FormattedMessage
              {...contentEditorIntelligencePanelMessages.addToGlossaryPrimaryLabel}
            />
          </FieldLabel>
          <Input
            id={primaryTermId}
        ...
        {translatable ? (
          <Field className="min-w-0 gap-1.5">
            <FieldLabel htmlFor={targetTermId}>
              <FormattedMessage
                {...contentEditorIntelligencePanelMessages.addToGlossaryTargetLabel}
              />
            </FieldLabel>
            <Input
              id={targetTermId}
        ...
        <Field className="gap-1.5">
          <FieldLabel htmlFor={definitionId}>
            <FormattedMessage
              {...contentEditorIntelligencePanelMessages.addToGlossaryDefinitionLabel}
            />
          </FieldLabel>
          <Textarea
            id={definitionId}
        ...
        <label className="flex min-w-0 items-center gap-2 text-sm text-foreground">
          ...
          />
          <FormattedMessage
            {...contentEditorIntelligencePanelMessages.addToGlossaryTranslatableLabel}
          />
        </label>
              ...
              render={
                <Button
                  ...
                  aria-label={intl.formatMessage(
                    contentEditorIntelligencePanelMessages.addToGlossaryPickerLabel,
                  )}
                  className={cn(
              ...
              <Command>
                <CommandInput
                  placeholder={intl.formatMessage(
                    contentEditorIntelligencePanelMessages.addToGlossaryPickerSearchPlaceholder,
                  )}
                />
                <CommandList
                ...
                >
                  <CommandEmpty>
                    {intl.formatMessage(
                      contentEditorIntelligencePanelMessages.addToGlossaryPickerEmpty,
                    )}
                  </CommandEmpty>
                  {glossaries.length > 0 ? (
                    <CommandGroup>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              <FormattedMessage
                {...contentEditorIntelligencePanelMessages.addToGlossaryCreateDialogTitle}
              />
            </DialogTitle>
          </DialogHeader>
          <FieldGroup>
            <Field className="gap-1.5">
              <FieldLabel htmlFor={newGlossaryNameId}>
                <FormattedMessage
                  {...contentEditorIntelligencePanelMessages.addToGlossaryCreateNameLabel}
                />
              </FieldLabel>
              <Input
          ...
          <DialogFooter>
            ...
            >
              <FormattedMessage
                {...contentEditorIntelligencePanelMessages.addToGlossaryCreateCancel}
              />
            </Button>
            <Button
              type="button"
            ...
            >
              <FormattedMessage
                {...contentEditorIntelligencePanelMessages.addToGlossaryCreateAction}
              />
            </Button>
          </DialogFooter>
        </DialogContent>
