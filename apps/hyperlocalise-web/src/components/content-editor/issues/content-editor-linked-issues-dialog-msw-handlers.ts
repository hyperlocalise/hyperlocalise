...
import {
  contentEditorLinkableIssuesFixture,
  contentEditorLinkedIssuesAssignableMembersFixture,
  contentEditorLinkedIssuesListFixture,
  contentEditorLinkedIssuesTranslationKeyId,
  type ContentEditorLinkedIssueListItemFixture,
...
function listResponse(issues: ContentEditorLinkedIssueListItemFixture[]) {
...
function linkedIssuesForRequest(request: Request) {
  ...
  if (translationKeyId) {
    return contentEditorLinkedIssuesListFixture.filter(
      (issue) => issue.translationKeyId === translationKeyId,
    );
  }

  const issues = contentEditorLinkableIssuesFixture.filter((issue) => {
    if (!search) {
      return true;
    }
...
export const contentEditorLinkedIssuesMswHandlers = [
  http.get(`${issueSheetBasePath}/assignable-members`, () =>
    HttpResponse.json(contentEditorLinkedIssuesAssignableMembersFixture),
  ),
  ...
  http.patch(`${issueSheetBasePath}/:issueId`, async ({ params, request }) => {
    ...
    const existing =
      contentEditorLinkableIssuesFixture.find((issue) => issue.id === params.issueId) ??
      contentEditorLinkedIssuesListFixture[0];
    return HttpResponse.json({
      issue: {
        ...existing,
        id: params.issueId,
        translationKeyId:
          body?.translationKeyId === null
            ...
            : typeof body?.translationKeyId === "string"
              ...
              : (existing?.translationKeyId ?? contentEditorLinkedIssuesTranslationKeyId),
    ...
    return HttpResponse.json(
      {
        issue: {
          ...
          status: "open",
          translationKeyId: body?.translationKeyId ?? contentEditorLinkedIssuesTranslationKeyId,
        },
      },
      { status: 201 },
    );
...
export const contentEditorLinkedIssuesEmptyMswHandlers = [
  http.get(`${issueSheetBasePath}/assignable-members`, () =>
    HttpResponse.json(contentEditorLinkedIssuesAssignableMembersFixture),
  ),
  http.get(issueSheetBasePath, ({ request }) => {
    ...
    return listResponse(
      contentEditorLinkableIssuesFixture.filter((issue) => issue.translationKeyId == null),
    );
  }),
  http.patch(`${issueSheetBasePath}/:issueId`, async ({ params, request }) => {
    const body = await readIssueSheetPatchBody(request);
...
export const contentEditorLinkedIssuesLoadingMswHandlers = [
  http.get(`${issueSheetBasePath}/assignable-members`, async () => {
    await delay("infinite");
    return HttpResponse.json(contentEditorLinkedIssuesAssignableMembersFixture);
  }),
...
export const contentEditorLinkedIssuesErrorMswHandlers = [
  http.get(`${issueSheetBasePath}/assignable-members`, () =>
    HttpResponse.json(contentEditorLinkedIssuesAssignableMembersFixture),
  ),
