...
export type ContentEditorIssueGuidanceStatus = {
  available: boolean;
  openIssueCount: number;
};

type ContentEditorIssueGuidanceListener = () => void;

let issueGuidanceStatus: ContentEditorIssueGuidanceStatus = EMPTY_CAT_ISSUE_GUIDANCE_STATUS;
const issueGuidanceListeners = new Set<ContentEditorIssueGuidanceListener>();
...
export function setCatIssueGuidanceStatus(status: ContentEditorIssueGuidanceStatus) {
  if (
...
export function subscribeCatIssueGuidance(listener: ContentEditorIssueGuidanceListener) {
