...
import { localizeCatMessageParityIssue } from "@/components/content-editor/message-format/content-editor-message-format-i18n";
...
type ContentEditorSegmentFixtureInput = Omit<
  ContentEditorSegment,
  "id" | "index" | "sourceLocale" | "targetLocale"
>;

const SOURCE_LOCALE = "en-US";
const TARGET_LOCALE = "vi";

const contentEditorSegmentInputs: ContentEditorSegmentFixtureInput[] = [
  {
...
export const contentEditorSegmentsFixture: ContentEditorSegment[] = contentEditorSegmentInputs.map(
  (segment, index) => ({
    ...segment,
...
export const contentEditorFormatChecksFixture: ContentEditorFormatCheck[] = [
  {
...
export const contentEditorIntelligenceFixture: ContentEditorSegmentIntelligence = {
  reviewReason:
...
export function createContentEditorWorkspaceState(
  ...
  return {
    ...
    segments,
    selectedSegmentId,
    formatChecks: contentEditorFormatChecksFixture,
    segmentFormatChecks: {
      [selectedSegmentId]: contentEditorFormatChecksFixture,
    },
...
export const contentEditorWorkspaceFixture = createContentEditorWorkspaceState();

export async function mockValidateFormat(
  segment: ContentEditorSegment,
  value: string,
  glossaryTerms: ContentEditorGlossaryTerm[] = contentEditorIntelligenceFixture.glossaryTerms,
): Promise<ContentEditorFormatCheck[]> {
  const checks = [...contentEditorFormatChecksFixture];
