...
import { QueueStatusDot } from "@/components/content-editor/segment/content-editor-segment-status";
...
function textForSegment(segment: ContentEditorVisualEditorSegment | undefined, fallback: string) {
  if (!segment) {
    return fallback;
  }
...
function TranslatableNode({
  ...
  segment: ContentEditorVisualEditorSegment;
  ...
  if (segments.length === 0) {
    return (
      <div className="flex min-h-64 items-center justify-center px-6 text-center text-sm text-neutral-500">
        <FormattedMessage {...contentEditorVisualEditorMessages.previewEmptyFile} />
      </div>
    );
  }
...
export function ContentEditorVisualEditorPreview({
  previewKind,
  segments,
  selectedSegmentId,
  highlightTranslatable,
