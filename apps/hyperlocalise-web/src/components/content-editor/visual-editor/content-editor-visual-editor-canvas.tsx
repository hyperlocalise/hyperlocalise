...
import type {
  ...
  ContentEditorVisualEditorPreviewKind,
...
export function ContentEditorVisualEditorCanvas({
  previewUrl,
  previewKind,
  device,
  onDeviceChange,
  ...
  device: ContentEditorVisualEditorDevice;
  onDeviceChange: (device: ContentEditorVisualEditorDevice) => void;
  ...
  return (
    <section
      ...
      aria-label={intl.formatMessage(contentEditorVisualEditorMessages.previewAria)}
    >
      <div className="flex flex-wrap items-center gap-3 border-b border-border bg-background px-3 py-2.5">
        ...
        <div className="flex items-center gap-2">
          ...
          <Label htmlFor="visual-editor-highlight" className="text-xs text-muted-foreground">
            <FormattedMessage {...contentEditorVisualEditorMessages.highlightToggle} />
          </Label>
        </div>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-auto p-4 sm:p-6">
        <div className={cn("mx-auto transition-[max-width]", DEVICE_WIDTH[device])}>
          <ContentEditorVisualEditorPreview
