...
import { useContentEditorEditorHotkeys } from "@/components/content-editor/editor/content-editor-editor-hotkeys";
...
import type {
  ContentEditorVisualEditorDevice,
  ContentEditorVisualEditorFilePage,
  ContentEditorVisualEditorFixture,
  ContentEditorVisualEditorSegment,
} from "./content-editor-visual-editor.types";
import { ContentEditorVisualEditorCanvas } from "./content-editor-visual-editor-canvas";
import { ContentEditorVisualEditorDetailPanel } from "./content-editor-visual-editor-detail-panel";
import { ContentEditorVisualEditorFilesSidebar } from "./content-editor-visual-editor-files-sidebar";
...
function needsAttention(status: ContentEditorSegmentStatus) {
...
function createFileState(page: ContentEditorVisualEditorFilePage): FileWorkspaceState {
  return {
...
): ContentEditorVisualEditorFilePage | null {
  return pagesBySourcePath[selectedSourcePath] ?? null;
}

export function ContentEditorVisualEditorWorkspace({
  initialState,
  className,
}: {
  initialState: ContentEditorVisualEditorFixture;
  className?: string;
}) {
  ...
  const [device, setDevice] = useState<ContentEditorVisualEditorDevice>("desktop");
  ...
  return (
    <div className={cn("flex h-full min-h-0 flex-col bg-background text-foreground", className)}>
      <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[17.5rem_minmax(0,1fr)_22rem] xl:grid-cols-[18rem_minmax(0,1fr)_24rem]">
        <ContentEditorVisualEditorFilesSidebar
        ...
        <ContentEditorVisualEditorCanvas
        ...
        <ContentEditorVisualEditorDetailPanel
      ...
      <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-border px-4 py-2 text-xs text-muted-foreground">
        <p>
          <FormattedMessage
            {...contentEditorVisualEditorMessages.footerCopyright}
            values={{ year: 2026 }}
          />
        </p>
        <div className="flex items-center gap-3">
          <span>
            <FormattedMessage
              {...contentEditorVisualEditorMessages.statusBarProgress}
              values={{ done: reviewedCount, total: segments.length }}
            />
          </span>
          <span>
            <FormattedMessage
              {...contentEditorVisualEditorMessages.statusBarRemaining}
        ...
        <div className="flex flex-wrap items-center gap-3">
          <span className="inline-flex items-center gap-1.5">
            ...
            <FormattedMessage {...contentEditorVisualEditorMessages.navigateHint} />
          </span>
          <span className="inline-flex items-center gap-1.5">
            <Kbd>tab</Kbd>
            <FormattedMessage {...contentEditorVisualEditorMessages.nextOpenHint} />
          </span>
          <span className="inline-flex items-center gap-1.5">
            <Kbd>esc</Kbd>
            <FormattedMessage {...contentEditorVisualEditorMessages.deselectHint} />
          </span>
        </div>
      </footer>
    </div>
