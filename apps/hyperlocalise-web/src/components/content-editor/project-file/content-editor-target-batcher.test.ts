...
const cell = (id: number, targetLocale = "fr") => ({
  sourcePath: "a.json",
  externalStringId: String(id),
  targetLocale,
});
const response = (body: ProjectFileCatTargetsInput) =>
  body.segments.map((segment) => ({
    ...segment,
    targets: Object.fromEntries(
      body.targetLocales.map((locale) => [
        locale,
        { text: locale + segment.externalStringId, isApproved: false, externalTranslationId: null },
      ]),
    ),
  }));
...
describe("native translation batches", () => {
  it("loads a 20 by 3 viewport with one request", async () => {
    ...
    const reads = Array.from({ length: 20 }, (_, i) =>
      ["fr", "de", "es"].map((locale) => load(cell(i, locale))),
    ).flat();
  ...
  it("bounds every rectangle and limits in-flight requests", async () => {
    ...
    const reads = Array.from({ length: 100 }, (_, i) =>
      Array.from({ length: 10 }, (_, j) => load(cell(i, "locale" + j))),
    ).flat();
    await vi.runAllTimersAsync();
