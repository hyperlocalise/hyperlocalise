...
type CommentsStoryHostProps = {
  initialSegment: ContentEditorSegment;
  isLoading?: boolean;
...
function CommentsStoryHost({
  ...
  async function handleAddComment(input: ContentEditorSegmentCommentInput) {
    setIsPostingComment(true);
    if (simulatePostDelayMs > 0) {
      await new Promise((resolve) => setTimeout(resolve, simulatePostDelayMs));
    }

    const nextComment: ContentEditorSegmentComment = {
...
const meta = {
  ...
  args: {
    ...
    isLoading: false,
    canAddComment: true,
    // Crowdin keeps Comment/Issue tabs; native uses ContentEditorEditorIssuesSection instead.
    supportsIssueComments: true,
  },
