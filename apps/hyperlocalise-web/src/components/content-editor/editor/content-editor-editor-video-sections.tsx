...
import { ContentEditorVideoPreview } from "./content-editor-video-preview";

function isVideoMode(segment: ContentEditorSegment) {
  return segment.contentKind === "video_file" || segment.contentKind === "video_url";
}

export function ContentEditorEditorVideoSourceSection({
  segment,
  canEdit,
  isBusy,
  onTreatAsVideo,
...
export function ContentEditorEditorVideoTargetSection({
  segment,
  canEdit,
  isBusy,
  isLoading,
  ...
  return (
    <section className="space-y-3">
      ...
      {isLoading ? (
        ...
        <ContentEditorVideoPreview
          src={previewSrc}
          emptyLabel={intl.formatMessage(contentEditorEditorPanelMessages.videoTargetEmpty)}
        />
      )}
