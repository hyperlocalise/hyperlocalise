...
export function ContentEditorQueueSkeletonList({
  ...
  return (
    <div className={cn("min-h-0 flex-1 overflow-hidden px-4 pb-3", className)}>
      <ul
        className="space-y-2"
        aria-busy="true"
        aria-label={intl.formatMessage(contentEditorQueuePanelMessages.loadingSegmentsAria)}
      >
        {Array.from({ length: rowCount }, (_, index) => (
          <li
            key={`content-editor-queue-skeleton-${index}`}
