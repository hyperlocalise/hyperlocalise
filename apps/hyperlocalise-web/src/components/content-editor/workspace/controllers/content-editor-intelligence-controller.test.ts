...
import type { ContentEditorSegmentConcordanceResult } from "@/components/content-editor/shared/dependencies";

import { createCatWorkspace } from "../content-editor-workspace-orchestrator";
import { ContentEditorIntelligenceController } from "./content-editor-intelligence-controller";
...
function createTestWorkspace(
  overrides: Parameters<typeof createContentEditorWorkspaceState>[0] = {},
) {
...
let activeController: ContentEditorIntelligenceController | undefined;

function createController(
  workspace = createTestWorkspace(),
  ports: ConstructorParameters<typeof ContentEditorIntelligenceController>[1] = { intl },
) {
  activeController?.dispose();
  const controller = new ContentEditorIntelligenceController(workspace, ports);
  controller.start();
...
describe("ContentEditorIntelligenceController", () => {
  describe("loadConcordance", () => {
    ...
    it("deduplicates concurrent concordance requests", async () => {
      let resolveLookup: ((value: ContentEditorSegmentConcordanceResult) => void) | undefined;
      const lookup = vi.fn(
        () =>
          new Promise<ContentEditorSegmentConcordanceResult>((resolve) => {
    ...
    it("returns cached intelligence without calling lookup again", async () => {
      const concordance: ContentEditorSegmentConcordanceResult = {
        glossaryTerms: [
    ...
    it("does not prefill empty targets from TM matches", async () => {
      ...
      } satisfies ContentEditorSegmentConcordanceResult);
      const { controller, workspace } = createController(undefined, {
        intl,
        services: { lookupSegmentConcordance: lookup },
      });
    ...
    it("ignores stale concordance results after reload", async () => {
      const resolvers: Array<(value: ContentEditorSegmentConcordanceResult) => void> = [];
      const lookup = vi.fn(
        () =>
          new Promise<ContentEditorSegmentConcordanceResult>((resolve) => {
