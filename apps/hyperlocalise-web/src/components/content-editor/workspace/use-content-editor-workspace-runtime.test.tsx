...
function renderController(
  ...
  return {
    ...
    ...renderHook(
      () =>
        useContentEditorWorkspaceRuntime({
          store,
          services: {
...
describe("useContentEditorWorkspaceRuntime", () => {
  ...
  it("clears concordance loading when AI review runs during an in-flight lookup", async () => {
    let resolveConcordance: ((value: ContentEditorSegmentConcordanceResult) => void) | undefined;
    const concordancePromise = new Promise<ContentEditorSegmentConcordanceResult>((resolve) => {
  ...
  it("keeps concordance results while format checks run in parallel", async () => {
    ...
    const validateFormat = vi
      ...
      .mockImplementation(
        () =>
          new Promise<ContentEditorFormatCheck[]>((resolve) => setTimeout(() => resolve([]), 50)),
      );
    const { result, store } = renderController(undefined, {
      services: {
        lookupSegmentConcordance,
  ...
  it("aborts superseded format validation during AI review", async () => {
    ...
    const resolvers: Array<(checks: ContentEditorFormatCheck[]) => void> = [];
  ...
  it("loads the next queue page when Next is pressed at the loaded boundary", () => {
    const firstPage = createContentEditorWorkspaceState({
      selectedSegmentId: "seg-02",
      segments: [
        {
          id: "seg-01",
