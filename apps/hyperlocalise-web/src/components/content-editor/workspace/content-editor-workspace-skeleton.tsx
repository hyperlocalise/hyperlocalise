...
function ContentEditorEditorPanelSkeleton() {
  ...
  return (
    <div
      ...
      aria-busy="true"
      aria-label={intl.formatMessage(contentEditorWorkspaceSkeletonMessages.loadingEditor)}
    >
      <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3 lg:px-5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Skeleton className="h-5 w-20 rounded-md bg-skeleton" />
        </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        <div className="mx-auto max-w-3xl space-y-6 px-4 py-5 sm:px-6 lg:space-y-7 lg:px-8 lg:py-8">
          <section className="space-y-3">
            ...
            <Skeleton className="h-20 w-full rounded-xl bg-skeleton" />
          </section>

          <section className="space-y-3">
            <Skeleton className="h-3 w-28 rounded-full bg-skeleton" />
            <Skeleton className="h-32 w-full rounded-xl bg-skeleton" />
          </section>

          <div className="flex flex-wrap gap-2">
            <Skeleton className="h-9 w-24 rounded-md bg-skeleton" />
            <Skeleton className="h-9 w-28 rounded-md bg-skeleton" />
            <Skeleton className="h-9 w-20 rounded-md bg-skeleton" />
          </div>

          <section className="space-y-3">
            <Skeleton className="h-3 w-32 rounded-full bg-skeleton" />
            <div className="space-y-0 divide-y divide-border rounded-xl border border-border bg-muted">
              {[0, 1, 2].map((item) => (
          ...
          <section className="space-y-3 border-t border-border pt-5">
            <div className="flex items-center justify-between gap-2">
              <Skeleton className="h-3 w-20 rounded-full bg-skeleton" />
              <Skeleton className="h-3 w-6 rounded-full bg-skeleton" />
            </div>
            <ul className="space-y-3">
              {[0, 1].map((item) => (
                <li key={item} className="space-y-2 rounded-lg border border-border p-3">
                  <Skeleton className="h-3 w-24 rounded-full bg-skeleton" />
...
function ContentEditorIntelligencePanelSkeleton() {
  const intl = useIntl();
  ...
  return (
    <div
      ...
      aria-busy="true"
      aria-label={intl.formatMessage(contentEditorWorkspaceSkeletonMessages.loadingIntelligence)}
    >
      <div className="shrink-0 border-b border-border px-4 py-3">
        <Skeleton className="h-4 w-32 rounded-full bg-skeleton" />
        <Skeleton className="mt-2 h-3 w-full max-w-xs rounded-full bg-skeleton" />
      </div>
      <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-4">
        {[0, 1, 2].map((item) => (
          <div key={item} className="space-y-2">
            <Skeleton className="h-3 w-28 rounded-full bg-skeleton" />
            <Skeleton className="h-16 w-full rounded-xl bg-skeleton" />
          </div>
        ))}
      </div>
    </div>
...
function ContentEditorQueuePanelSkeleton() {
  const intl = useIntl();
  ...
  return (
    <div
      ...
      aria-busy="true"
      aria-label={intl.formatMessage(contentEditorWorkspaceSkeletonMessages.loadingQueue)}
    >
      <div className="shrink-0 border-b border-border px-4 py-3">
        <Skeleton className="h-4 w-16 rounded-full bg-skeleton" />
      </div>
      ...
      <div className="shrink-0 px-4 py-3">
        <Skeleton className="h-1.5 w-full rounded-full bg-skeleton" />
      </div>

      <ContentEditorQueueSkeletonList />

      <div className="flex shrink-0 items-center justify-between gap-2 border-t border-border px-4 py-3">
        <Skeleton className="h-3 w-28 rounded-full bg-skeleton" />
        <div className="flex items-center gap-1">
          <Skeleton className="h-8 w-16 rounded-md bg-skeleton" />
          <Skeleton className="h-8 w-12 rounded-md bg-skeleton" />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <div className="mx-4 mt-3 grid h-10 grid-cols-3 gap-1 rounded-lg bg-muted p-1">
        <Skeleton className="h-full rounded-md bg-skeleton" />
