---
id: gitlab-repo-agent
sharedSkills: recent-source-changes
---

## GitLab repository agent procedure
...
- Start with `repoGitState` only when you need the current branch/HEAD. Then use `gitHistory` for the requested window: last 24 hours for a daily lookback, or the scheduled lookback window.
...
- Follow the customer instructions exactly, including delivery channel (Slack, email) and any code-layer review focus.
