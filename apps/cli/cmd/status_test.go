...
func TestStatusCommandRecognizesSubtitleFiles(t *testing.T) {
	dir := t.TempDir()
	configPath := filepath.Join(dir, "i18n.jsonc")
  ...
  "llm": {"profiles":{"default":{"provider":"openai","model":"gpt-4.1-mini","prompt":"Translate"}}}
	...
	if got := out.String(); !strings.Contains(got, "srt.0001,") || !strings.Contains(got, ",fr,translated,unknown,") {
		t.Fatalf("expected translated srt status row, got: %s", got)
	}
}
