...
func TestRunDryRunRecognizesSubtitleFiles(t *testing.T) {
	dir := t.TempDir()
	configPath := filepath.Join(dir, "i18n.jsonc")
	sourcePath := filepath.Join(dir, "content", "en.srt")
	targetPath := filepath.Join(dir, "dist", "fr.srt")
...
func TestRunDryRunSupportsYAMLFiles(t *testing.T) {
	...
	if err := cmd.Execute(); err != nil {
		t.Fatalf("run command yaml dry-run: %v", err)
	}
	if !strings.Contains(out.String(), "dry_run=true") || !strings.Contains(out.String(), "home.title") {
		t.Fatalf("expected yaml dry-run output, got %q", out.String())
	}
	if _, err := os.Stat(targetPath); !os.IsNotExist(err) {
		t.Fatalf("expected no target file written in dry-run, stat err=%v", err)
	}
...
func TestRunDryRunWarnsWhenLegacyPromptIsConfigured(t *testing.T) {
	...
	if !strings.Contains(out.String(), "warning=legacy_prompt profile=default") {
		t.Fatalf("expected legacy prompt warning, got %q", out.String())
	}
	if !strings.Contains(out.String(), `message="llm.profiles.default.prompt is deprecated; migrate to system_prompt and user_prompt"`) {
		t.Fatalf("expected quoted warning message, got %q", out.String())
	}
}

func TestRunDryRunSkipsLegacyWarningWhenNewPromptFieldsAreSet(t *testing.T) {
	dir := t.TempDir()
	configPath := filepath.Join(dir, "i18n.jsonc")
	sourcePath := filepath.Join(dir, "content", "en", "strings.json")
	targetPath := filepath.Join(dir, "dist", "fr", "strings.json")
	...
	content := `{
	  ...
	  "buckets": {"ui":{"files":[{"from":"` + filepath.ToSlash(sourcePath) + `","to":"` + filepath.ToSlash(targetPath) + `"}]}},
	  "groups": {"default":{"targets":["fr"],"buckets":["ui"]}},
	  "llm": {"profiles":{"default":{"provider":"openai","model":"gpt-4.1-mini","prompt":"Translate {{input}}","system_prompt":"custom system","user_prompt":"custom user"}}}
	}`
	if err := os.WriteFile(configPath, []byte(content), 0o600); err != nil {
		t.Fatalf("write config: %v", err)
	}
	...
	if strings.Contains(out.String(), "warning=legacy_prompt profile=default") {
		t.Fatalf("expected legacy prompt warning to be suppressed, got %q", out.String())
	}
}
