...
func TestRunDeletedOnlySourceEntryKeysProduceZeroTasks(t *testing.T) {
	svc := newTestService()
	sourcePath := "/tmp/source.json"
	targetPath := "/tmp/out.json"
	svc.loadConfig = func(_ string) (*config.I18NConfig, error) {
	...
	report, err := svc.Run(context.Background(), Input{
		...
		SourceEntryKeys: map[string][]string{sourcePath: {"deleted"}},
	})
	if err != nil {
		t.Fatalf("run dry-run: %v", err)
	}
	if report.PlannedTotal != 0 || report.ExecutableTotal != 0 {
		t.Fatalf("expected zero planned tasks for deleted-only keys, got %+v", report)
	}
}
...
func (c *memoryExactCache) Get(_ context.Context, key string) (string, bool, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *memoryExactCache) Put(_ context.Context, entry cache.ExactCacheWrite) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
type failingWriteExactCache struct {
	values map[string]string
}

func (c *failingWriteExactCache) Get(_ context.Context, key string) (string, bool, error) {
	value, ok := c.values[key]
	return value, ok, nil
}

func (c *failingWriteExactCache) Put(_ context.Context, _ cache.ExactCacheWrite) error {
...
func TestRunL1ExactCacheSkipsProviderForSameDimensions(t *testing.T) {
	...
	l1 := &memoryExactCache{values: map[string]string{}}
	svc.newCache = func(_ config.CacheConfig) (*cache.Service, error) {
		return &cache.Service{Enabled: true, L1: l1}, nil
	}
	...
	if _, err := svc.Run(context.Background(), Input{Force: true}); err != nil {
	...
	if translateCalls != 1 {
		t.Fatalf("expected second run to hit L1 and skip provider; calls=%d", translateCalls)
	}
}

func TestRunL1ExactCacheKeyChangesWhenModelChanges(t *testing.T) {
	svc := newTestService()
	sourcePath := "tests/json/en-US.json"
	targetPath := "/tmp/out.json"
	model := "gpt-4.1-mini"
	...
	if translateCalls != 2 {
		t.Fatalf("expected model change to miss L1 and call provider again; calls=%d", translateCalls)
	}
}

func TestRunL1ExactCacheKeyChangesWhenSourceLocaleChanges(t *testing.T) {
	svc := newTestService()
	sourcePath := "tests/json/source.json"
	targetPath := "/tmp/out.json"
	sourceLocale := "en-US"
	svc.loadConfig = func(_ string) (*config.I18NConfig, error) {
		cfg := testConfig(sourcePath, targetPath)
		cfg.Locales.Source = sourceLocale
		cfg.Cache = config.CacheConfig{Enabled: true, L1: config.CacheTierConfig{Enabled: true}}
		return &cfg, nil
	}
	svc.readFile = func(path string) ([]byte, error) {
	...
	if translateCalls != 2 {
		t.Fatalf("expected source-locale change to miss L1 and call provider again; calls=%d", translateCalls)
	}
}

func TestRunL1ExactCacheNormalizationIgnoresEquivalentSourceWhitespace(t *testing.T) {
	svc := newTestService()
	sourcePath := "tests/json/en-US.json"
	targetPath := "/tmp/out.json"
	first := true
	...
	if translateCalls != 1 {
		t.Fatalf("expected equivalent normalized source to hit L1; calls=%d", translateCalls)
	}
}

func TestRunL1ExactCacheKeyChangesWhenPromptTemplateChanges(t *testing.T) {
	svc := newTestService()
	sourcePath := "tests/json/en-US.json"
	targetPath := "/tmp/out.json"
	promptTemplate := "Translate {{source}} to {{target}}: {{input}}"
	svc.loadConfig = func(_ string) (*config.I18NConfig, error) {
		cfg := testConfig(sourcePath, targetPath)
		profile := cfg.LLM.Profiles["default"]
		profile.Prompt = promptTemplate
		cfg.LLM.Profiles["default"] = profile
		cfg.Cache = config.CacheConfig{Enabled: true, L1: config.CacheTierConfig{Enabled: true}}
		return &cfg, nil
	}
	...
	promptTemplate = "Translate carefully {{source}} to {{target}}: {{input}}"
	if _, err := svc.Run(context.Background(), Input{Force: true}); err != nil {
		t.Fatalf("second run: %v", err)
	}
	if translateCalls != 2 {
		t.Fatalf("expected prompt-template change to miss L1; calls=%d", translateCalls)
	}
}

func TestRunL1ExactCacheFormatJSModeDetectedByContent(t *testing.T) {
	svc := newTestService()
	sourcePath := "tests/misc/en-US.json"
	targetPath := "/tmp/out.json"
	svc.loadConfig = func(_ string) (*config.I18NConfig, error) {
		cfg := testConfig(sourcePath, targetPath)
		cfg.Cache = config.CacheConfig{Enabled: true, L1: config.CacheTierConfig{Enabled: true}}
	...
	svc.readFile = func(path string) ([]byte, error) {
		switch path {
		case sourcePath:
			return []byte(`{"hello":{"defaultMessage":"Hello"}}`), nil
		case targetPath:
			return []byte(`{}`), nil
		default:
			return nil, filepath.ErrBadPattern
	...
	if translateCalls != 1 {
		t.Fatalf("expected strict formatjs JSON to be consistently detected and hit L1; calls=%d", translateCalls)
	}
}

func TestRunL1ExactCacheKeyChangesWhenContextMemoryChanges(t *testing.T) {
	keyA := exactCacheKey(Task{
		SourceLocale:    "en",
		TargetLocale:    "fr",
		SourceText:      "Hello",
		ProfileName:     "default",
	...
	keyB := exactCacheKey(Task{
		...
		RAGSnapshot:     "r",
		ContextKey:      "file:1",
		ContextMemory:   "memory-b",
	})
	if keyA == keyB {
		t.Fatal("expected context-memory fingerprint to affect exact cache key")
	}
}
...
func TestRunCacheWriteFailureDoesNotFailTask(t *testing.T) {
	...
	svc.newCache = func(_ config.CacheConfig) (*cache.Service, error) {
		return &cache.Service{Enabled: true, L1: &failingWriteExactCache{values: map[string]string{}}}, nil
	}
	svc.translate = func(_ context.Context, req translator.Request) (string, error) {
		return "FR(" + req.Source + ")", nil
	}
