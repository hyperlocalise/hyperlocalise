...
If your bucket mappings point to `.md` files, `hyperlocalise run --dry-run` still plans tasks from Markdown sources.

When you run without `--dry-run`, output keeps Markdown layout stable while translating extracted text nodes.
