...
description: "Localise Android strings.xml files, plurals, and placeholders through Hyperlocalise Platform."
---

This tutorial connects Android `strings.xml` resources to Hyperlocalise Platform. The CLI translates `<string>` and `<plurals>` resources while preserving comments, namespaces, attributes, and Android placeholders.
...
- a Platform project with `en` as source and `fr` and `de` as targets; and
