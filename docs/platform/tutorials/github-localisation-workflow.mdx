...
description: "Move source files from a pull request through Platform review and return approved translations to git."
---

This tutorial connects a GitHub repository to Hyperlocalise Platform. Engineers keep source content in git, reviewers approve language in Platform, and approved translations return through a pull request.
...
The dry run validates configuration and shows which source files the merge would upload. It does not change Platform.
...
`sync push` uploads source files only. It does not start a translation job. In Platform, add a **Source upload** automation or create a request after the upload.

## 5. Review and approve in Platform
