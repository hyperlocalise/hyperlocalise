...
GitLab.com stays on Pipes. For a self-hosted instance, open **Integrations** and add a **Self-hosted GitLab** connection with a public HTTPS URL and a personal access token (`read_api` and `read_repository`). Hyperlocalise rejects `gitlab.com` as a self-hosted URL. Self-hosted hosts must be reachable over public HTTPS (no loopback).
