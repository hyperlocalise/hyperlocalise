...
Open **Integrations** → **Ahrefs** and connect through the WorkOS Pipes widget.

Ahrefs is not in the default WorkOS catalog. Enable a custom API-key provider with slug `ahrefs` in the WorkOS Dashboard, and allow the widget origin for this app. Users paste an Ahrefs MCP API key (Account Settings → API Keys). WorkOS stores the secret; Hyperlocalise never writes it to `ahrefs_connections`.
...
3. At run time the automation vends that user's Pipes credential and calls `https://api.ahrefs.com/mcp/mcp` with `Authorization: Bearer`.

The org-level `ahrefs_connections` table is deprecated and unused. Do not add new callers.

## MCP server connections

Add a remote MCP server when an [automation](/platform/automations) should call external tools at run time (for example Semrush). Ahrefs uses [WorkOS Pipes](/platform/integrations#ahrefs) instead of a generic MCP server connection.
