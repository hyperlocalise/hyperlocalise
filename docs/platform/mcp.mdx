...
`zernio_create_ad` requires `accountId`, `adAccountId`, and `name`. Optional fields include `goal`, `budgetAmount`, `budgetType`, `headline`, `body`, `linkUrl`, `imageUrl`, `callToAction`, `status`, `existingCampaignId`, `adSetId`, `targeting`, `extra`, `idempotencyKey`, and `validateOnly`.
