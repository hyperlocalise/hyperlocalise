...
To let a **saved automation** call a **remote** MCP server (Semrush or your own), add an **MCP Server** connection under **Integrations**, then enable the MCP tool on the automation. Ahrefs uses [WorkOS Pipes](/platform/integrations#ahrefs) instead. That path is outbound from Hyperlocalise to a third-party server and is unrelated to the hosted `/api/mcp` server agents connect to.
