---
title: "MCP server"
description: "Connect coding agents to Hyperlocalise Platform over the Model Context Protocol."
---
...
Replace the host with your Platform origin when developing locally, for example `http://localhost:3000/mcp`.
...
To let a **saved automation** call a **remote** MCP server (Semrush or your own), add an **MCP Server** connection under **Integrations**, then enable the MCP tool on the automation. For Ahrefs, use the dedicated [Ahrefs connection](/platform/integrations#other-external-services) instead. That path is outbound from Hyperlocalise to a third-party server and is unrelated to the hosted `/api/mcp` server agents connect to.
