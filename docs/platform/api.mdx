...
The public API is the machine edge of Hyperlocalise Platform. It is the contract the [CLI](/platform/cli) uses to upload sources and download translations. Use it from your own scripts when you need the same operations without the CLI.

There is no published OpenAPI spec yet. The routes below are the `/api/v1` surface Platform actually serves.
...
| `projectId` | yes | Platform project id |
...
The CLI does not call this endpoint during `sync push`. Platform automations and the UI create jobs after upload.
...
To export only the segments that match Content Editor filters (CSV, TMX, XLIFF, XLSX), use **Download** in the [Content Editor](/platform/cat) queue—not this endpoint.
