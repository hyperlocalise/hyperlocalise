...
On a project **Automations** page, **Content sync** moves files between a connected source and that project. Integrations only connects the account.

1. Connect GitHub, GitLab, Contentful, or Intercom under **Integrations**.
2. Open the project **Automations** page.
3. Click **Add content sync**, choose the resource and folders, then enable **Sync content**.

Pull copies source into the project folder. Push writes translations that are already ready back to the provider (a pull request for git, drafts for a CMS). Content sync does not create translation jobs and does not use an agent prompt.

GitHub is the first executable provider. Contentful, GitLab, and Intercom share the same setup; their movers follow.
...
When the `automation_workflow` Autumn feature is enabled, organization **Automations** also shows **Visual workflows**. These are deterministic graphs (manual trigger, HTTP requests, if/else branching, and AI steps) rather than LLM-orchestrated playbooks.
