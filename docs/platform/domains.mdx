...
When linking a domain, the user provides only the hostname. After ownership verification succeeds, Hyperlocalise reads locale metadata and checks those markets plus the four defaults through DataForSEO Labs. Each market is requested separately; users review and save the resulting market selection.
