...
Native workspace **glossaries** support interchange through Platform. Provider-linked glossaries are read-only in Platform and follow different rules.
...
1. Open a native glossary, or a **mirrored** provider glossary stored in Hyperlocalise (a synced copy stored in Platform).
...
Live Crowdin glossaries are read-only in Platform and cannot be exported from the UI.
