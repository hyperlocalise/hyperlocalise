...
For Apple/Xcode Stringsdict (`.stringsdict`), `run` preserves plist/XML structure (including plural branches) while replacing `<string>` values.

For Apple/Xcode Strings Catalog (`.xcstrings`), `run` translates localized `stringUnit.value` entries while preserving catalog metadata/state fields and variation structure.
