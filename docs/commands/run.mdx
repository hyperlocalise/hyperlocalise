...
For HTML (`.html`), `run` translates text content inside block-level elements and preserves non-translatable structure:

- `<script>`, `<style>`, and `<head>` content
- inline tags within translatable segments (`<strong>`, `<em>`, `<a>`, etc.) — these are protected as placeholders and restored verbatim after translation
- HTML entities (`&amp;`, `&lt;`, etc.)
- HTML comments
