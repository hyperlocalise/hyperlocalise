...
```bash
hyperlocalise run [--config <path>] [--group <name>] [--bucket <name>] [--diff] [--target-locale <locale>] [--dry-run] [--workers <count>] [--output <report.json>] [--experimental-context-memory] [--context-memory-scope <file|bucket|group>] [--context-memory-max-chars <count>]
```
...
- `--diff`: run only tasks for files changed in the current git worktree; JSON-like files (`.json`, `.jsonc`, `.arb`) are narrowed to changed keys when possible
...
## Scope runs to the current worktree diff

Use `--diff` when you want to translate only source files changed in your current git worktree. For JSON-like source files, `run` narrows the plan to changed keys when it can compare the current file to `HEAD`.

```bash
hyperlocalise run --diff --dry-run
```

`--diff` includes both staged and unstaged changes, plus untracked files. Deleted source files are ignored. `--diff` cannot be combined with `--prune`.
