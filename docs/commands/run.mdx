...
```bash
hyperlocalise run [--config <path>] [--group <name>] [--bucket <name>] [--dry-run] [--workers <count>] [--output <report.json>] [--experimental-context-memory] [--experimental-auto-repair] [--context-memory-scope <file|bucket|group>] [--context-memory-max-chars <count>]
```
...
- `--experimental-auto-repair`: enable two-pass translation with automatic repair when source-language leakage is detected
