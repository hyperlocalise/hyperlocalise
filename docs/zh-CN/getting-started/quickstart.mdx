---
title: "快速入门"
description: "大约10分钟内完成端到端的超本地化运行。"
---

## 目标

创建一个 `i18n.jsonc`安全运行翻译规划，生成输出，并审查状态。
...
`hyperlocalise` 自动从读取凭据 `.env` 和 `.env.local` 在您当前的项目目录中。导出的 shell 变量仍然具有优先权。
...
这会写入翻译并进行更新 `.hyperlocalise.lock.json`。详情请参见 [锁文件合约](/reference/lockfile-contract).
...
- 调整配置于 [i18n 配置](/configuration/i18n-config).
- 使用 与您的 TMS 同步 [同步推送](/commands/sync-push) 和 [同步拉取](/commands/sync-pull).
