...
每个语言环境的 token 使用会输出为：`locale_usage locale=<locale> prompt_tokens=<...> completion_tokens=<...> total_tokens=<...>`。

传入 `--output` 时，JSON 报告包含运行元数据（`generatedAt`、`configPath`）、聚合 token 使用、按语言环境统计的使用量，以及按条目的批次使用量。
