...
适用于 Apple/Xcode Stringsdict (`.stringsdict`), `run` 保留 plist/XML 结构 (包括复数分支) 替换时 `<string>` 值。

用于 Apple/Xcode 字符串目录 (`.xcstrings`), `run` 翻译本地化 `stringUnit.value` 在保留目录元数据/状态字段和变体结构的同时处理条目。
