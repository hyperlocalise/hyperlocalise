...
- `--locale`：仅为给定的目标区域设置运行任务（可重复）；`--target-locale` 是一个别名
- `--dry-run`：仅打印计划，不要翻译或写入文件
- `--force`：重新运行所有计划任务并忽略锁文件跳过状态
- `--prune`：移除源文件中不再存在的目标键
- `--prune-max-deletions`：在需要显式覆盖之前，单次运行中删除的最大过期密钥数（默认 `100`）
...
- `user_prompt` 用于负载内容（要翻译的文本，或用于上下文记忆总结的源内容）。
- 翻译流程支持配置文件`user_prompt`覆盖。
- 上下文记忆摘要流程始终使用内置的摘要负载模板，并且不应用配置文件 `user_prompt` 覆盖。
...
- 新作用域中的首个条目会等待内存生成完成。
- 同一作用域中的后续条目会重用现有的作用域内存，并在不重新构建的情况下继续执行。
...
当你想在不更改分组或桶选择的情况下只重新运行特定区域设置时，请使用 `--locale`。你可以重复该标志以选择多个区域设置。为兼容较旧的脚本，同样的过滤器也可作为 `--target-locale` 使用。

```bash
hyperlocalise run --group tests --locale fr --locale de --dry-run
```

如果请求的语言区域不在`locales.targets`中，`run`会因`unknown target locale`规划错误而失败。与`--group`结合使用时，只会规划属于该组的语言区域。

当与 `--prune` 结合使用时，过期键检测也仅限于所选目标语言环境。`run` 只会扫描并清理属于筛选后语言环境集合的目标文件。
...
使用 `--force` 来忽略锁文件跳过状态，并再次执行每个计划中的任务。
...
每个区域设置的令牌使用情况打印为：`locale_usage locale=<locale> prompt_tokens=<...> completion_tokens=<...> total_tokens=<...>`。

当你传递`--output`时，JSON 报告会包含运行元数据（`generatedAt`、`configPath`）、汇总的 token 使用量、按语言环境的使用量以及按条目的批处理使用量。
...
在提供商速率限制下或在受限的 CI 环境中运行时，降低 `--workers`。先从 `1` 开始以稳定重试，然后逐步增加。

当你的提供商配额和机器资源允许更高吞吐量时，提高 `--workers`。请小幅度增加，并观察 API 错误率以及本地 CPU 和内存使用情况。
