...
- `--locale`: chỉ chạy các tác vụ cho ngôn ngữ mục tiêu đã cho (có thể lặp lại); `--target-locale` là một bí danh
...
- `system_prompt` được dùng cho hướng dẫn và ngữ cảnh thực thi.
...
- Luồng tóm tắt bộ nhớ ngữ cảnh luôn sử dụng mẫu dữ liệu tóm tắt tích hợp sẵn và không áp dụng ghi đè `user_prompt` của hồ sơ.
...
## Phạm vi áp dụng cho một ngôn ngữ mục tiêu

Dùng `--locale` khi bạn muốn chỉ chạy lại các locale cụ thể mà không thay đổi lựa chọn group hoặc bucket. Bạn có thể lặp lại cờ này để chọn nhiều locale. Bộ lọc tương tự cũng có sẵn dưới dạng `--target-locale` để tương thích với các script cũ hơn.

```bash
hyperlocalise run --group tests --locale fr --locale de --dry-run
```

Nếu một locale được yêu cầu không có trong `locales.targets`, `run` sẽ thất bại với lỗi lập kế hoạch `unknown target locale`. Khi kết hợp với `--group`, chỉ các locale thuộc nhóm đó mới được lập kế hoạch.

Khi kết hợp với `--prune`, việc phát hiện khóa cũ cũng chỉ bị giới hạn trong các ngôn ngữ đích đã chọn. `run` chỉ quét và loại bỏ các tệp đích thuộc tập ngôn ngữ đã được lọc.
...
Việc sử dụng token theo từng ngôn ngữ được in dưới dạng: `locale_usage locale=<locale> prompt_tokens=<...> completion_tokens=<...> total_tokens=<...>`.

Khi bạn truyền `--output`, báo cáo JSON sẽ bao gồm siêu dữ liệu của lần chạy (`generatedAt`, `configPath`), tổng mức sử dụng token, mức sử dụng theo từng ngôn ngữ và mức sử dụng theo từng lô cho mỗi mục.
...
Hạ `--workers` khi bạn chạm đến giới hạn tốc độ của nhà cung cấp hoặc chạy trong các môi trường CI bị hạn chế. Bắt đầu với `1` để ổn định các lần thử lại, rồi tăng dần.

Tăng `--workers` khi hạn ngạch nhà cung cấp và tài nguyên máy cho phép thông lượng cao hơn. Hãy tăng từng bước nhỏ và theo dõi tỷ lệ lỗi API cùng mức sử dụng CPU và bộ nhớ cục bộ.
