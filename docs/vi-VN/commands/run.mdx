...
- `--output`: ghi báo cáo chạy JSON dạng máy đọc được vào đường dẫn chỉ định
...
Mức sử dụng token theo locale được in theo dạng: `locale_usage locale=<locale> prompt_tokens=<...> completion_tokens=<...> total_tokens=<...>`.

Khi truyền `--output`, báo cáo JSON bao gồm metadata lần chạy (`generatedAt`, `configPath`), tổng mức sử dụng token, mức sử dụng theo locale, và mức sử dụng theo từng batch mục nhập.
