...
Dành cho Apple/Xcode Stringsdict (`.stringsdict`), `run` giữ nguyên cấu trúc plist/XML (bao gồm các nhánh số nhiều) trong khi thay thế `<string>` giá trị.

Dành cho Danh mục Chuỗi của Apple/Xcode (`.xcstrings`), `run` dịch bản địa hóa `stringUnit.value` các mục trong khi vẫn giữ nguyên các trường siêu dữ liệu/trạng thái của danh mục và cấu trúc biến thể.
