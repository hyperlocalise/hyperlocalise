...
<Note>
Việc thay đổi cấu trúc prompt (ví dụ: chuyển ngữ cảnh từ tin nhắn của người dùng sang tin nhắn hệ thống) không tự động làm mất hiệu lực các bản dịch đã được lưu trong bộ nhớ đệm. Để buộc dịch lại sau khi tái cấu trúc prompt, hãy tăng `prompt_version` trong hồ sơ của bạn.
</Note>
...
- Các mục nhập sau đó trong cùng phạm vi sẽ tái sử dụng bộ nhớ đã được lưu đệm và tiếp tục mà không cần xây dựng lại.
