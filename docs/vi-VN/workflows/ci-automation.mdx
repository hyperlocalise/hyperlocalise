...
```bash
hyperlocalise sync pull --output json --dry-run=true
hyperlocalise sync push --output json --dry-run=true
```
...
jobs:
  pull-sync:
    ...
    steps:
      ...
      - run: go run ./apps/cli sync pull --output json --dry-run=true
```

Để áp dụng các thay đổi từ xa vào các tệp cục bộ trong CI, chuyển sang:

```bash
go run ./apps/cli sync pull --output json --dry-run=false
```
  ...
  push-sync:
    ...
    steps:
      ...
      - uses: actions/setup-go@v5
        with:
          go-version-file: go.mod
      - run: go run ./apps/cli sync push --output json --dry-run=false
```

Đối với một lần kiểm tra xác thực không làm thay đổi, hãy sử dụng:

```bash
go run ./apps/cli sync push --output json --dry-run=true
```
