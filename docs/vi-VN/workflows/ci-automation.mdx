...
`sync pull` và `sync push` mặc định áp dụng thay đổi. Thêm `--dry-run` cho các tác vụ xem trước CI không làm thay đổi.
...
Chỉ bỏ `--dry-run` trong các tác vụ được ủy quyền rõ ràng để ghi tệp cục bộ hoặc hệ thống từ xa.
...
jobs:
  pull-sync:
    ...
    steps:
      ...
      - run: go run ./apps/cli sync pull --output json --dry-run
```

Để áp dụng các thay đổi từ xa vào các tệp cục bộ trong CI, chuyển sang:

```bash
go run ./apps/cli sync pull --output json
```
  ...
  push-sync:
    ...
    steps:
      ...
      - uses: actions/setup-go@v5
        with:
          go-version-file: go.mod
      - run: go run ./apps/cli sync push --output json
```

Đối với một lần kiểm tra xác thực không làm thay đổi, hãy sử dụng:

```bash
go run ./apps/cli sync push --output json --dry-run
```
