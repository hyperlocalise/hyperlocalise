...
description: "Set credentials for OpenAI, Azure OpenAI, Anthropic, Gemini, Bedrock, LM Studio, Ollama, Groq, OpenRouter, Vercel AI Gateway, Google Cloud Translation, DeepL, and remote TMS adapters."
---
...
Unlike LLM providers, Hyperlocalise does not read a fixed environment variable name for MT credentials. Each `mt.profiles.<name>` picks its own variable name via `api_key_env`, so the names below are examples — any name works as long as it matches `api_key_env` in `i18n.yml`.
...
Reference the variable name you chose from `mt.profiles.<name>.api_key_env`:
...
DeepL also requires `mt.profiles.<name>.base_url` to be set to exactly one of DeepL's two API hosts:

- `https://api.deepl.com` for a DeepL Pro account
- `https://api-free.deepl.com` for a DeepL Free account
