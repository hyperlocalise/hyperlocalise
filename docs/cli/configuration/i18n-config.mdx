...
When set, commands that load the config (`run`, `check`, `status`, `pack`, `sync`, and others) fail if the running CLI version does not match the pinned value. The `hl` alias is also accepted (`hl@1.12.1`).
...
See [File formats](/cli/reference/formats/overview) for the complete list of supported extensions, source examples, and writeback limitations.
