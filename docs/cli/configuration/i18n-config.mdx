...
When `hyperlocalise` is absent, set `storage.adapter` so `sync push` and `sync pull` use TMS entry sync. Keep `storage` for provider-specific file commands as well.
