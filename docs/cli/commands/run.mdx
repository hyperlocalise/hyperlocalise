...
For lockfile fields, lifecycle, and reset guidance, see [Lockfile contract](/cli/reference/lockfile-contract).
...
For SubRip and WebVTT subtitles (`.srt`, `.vtt`), `run` translates cue payload text and preserves non-translatable structure:

- cue numbers, timestamps, and positioning or cue settings
- WebVTT `WEBVTT` headers, `NOTE`, `STYLE`, and `REGION` blocks
- HTML-like markup inside cues such as `<i>` and `<b>`
- WebVTT cue spans such as `<c.red>`, `<v Speaker>`, and karaoke timestamps like `<00:01.000>`

Cues are exposed as sequential keys such as `srt.0001` and `vtt.0001`. Timestamp lines are available as entry context. Writeback replaces cue text in place and ignores extra translation keys so cue counts stay aligned with the source file. Blank lines inside translated cue text are dropped so they cannot split cues.
...
MT usage is JSON-only (not printed to stdout) and appears under `sourceChars`/`translatedChars`/`requestCount`/`durationMs` at the run level, `localeMTUsage` (per target locale), and `mtUsageByProfile` (per MT profile). See "Machine translation (MT) execution" above.
...
- [eval](/cli/commands/eval)
- [status](/cli/commands/status)
- [sync push](/cli/commands/sync-push)
- [sync pull](/cli/commands/sync-pull)
- [Lockfile contract](/cli/reference/lockfile-contract)

## Related guides

- [Translate only new and changed strings](/cli/workflows/translate-changed-strings).
