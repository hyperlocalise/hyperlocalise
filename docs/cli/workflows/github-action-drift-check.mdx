...
When source files use [`extract --prefix-id`](/cli/commands/extract) keys but target files use packed ids from [`pack --prefix-id`](/cli/commands/pack), set `prefix-id: true` so `check` strips source prefixes before comparing keys. This also applies to changed keys when `github-diff` is enabled.
...
- `prefix-id`: in `check` mode, pass [`check --prefix-id`](/cli/commands/check) so prefixed source keys are compared against packed target ids. Only supported when `check` is `check`. Defaults to `false`.
