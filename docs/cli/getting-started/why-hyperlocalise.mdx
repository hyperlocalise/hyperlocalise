...
4. Optional curation path: use [`sync push` (experimental)](/cli/commands/sync-push), curate in TMS, then [`sync pull` (experimental)](/cli/commands/sync-pull).
...
Use [Hyperlocalise Cloud](/) when your team wants infrastructure for multilingual content operations without running the CLI.
...
TMS curation path: [Storage overview](/cli/storage/overview) -> [`sync push` (experimental)](/cli/commands/sync-push) -> [TMS curation loop (experimental)](/cli/workflows/tms-curation-loop) -> [`sync pull` (experimental)](/cli/commands/sync-pull)
