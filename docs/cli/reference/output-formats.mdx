...
When `run` writes `.srt` or `.vtt` translation targets, cue numbers, timestamps, positioning, WebVTT headers, and `NOTE` / `STYLE` / `REGION` blocks come from the selected template. Only cue payload text is replaced. Extra translation keys are ignored so cue counts stay aligned with the template.
