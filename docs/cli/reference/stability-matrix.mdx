...
| Translation routing / MT | `translation.default`/`translation.rules` + `mt.profiles` (Google, DeepL) | Experimental | Only `google` and `deepl` are wired up; `mt.profiles` has unused Microsoft/Amazon-shaped fields. MT has no automatic fallback to an LLM profile on failure. |
