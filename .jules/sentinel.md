...
## 2026-06-20 - [Enhancement] Universal Adoption of providerSafeFetch for Outbound Requests
**Vulnerability:** Several backend components (Slack agent, LLM provider validation, Resend attachment downloader) still used the global `fetch` instead of `providerSafeFetch`. This bypassed the application's DNS-based SSRF protections, potentially allowing access to internal metadata services or restricted network segments if those components could be influenced by user input (e.g., malicious Slack workspace, custom LLM base URLs, or crafted Resend attachment URLs).
**Learning:** Security utilities like `providerSafeFetch` are only effective if they are used consistently across all outbound network boundaries. Partial adoption leaves gaps that attackers can target.
**Prevention:** Audit the codebase for all instances of the global `fetch` in backend-executed code and replace them with `providerSafeFetch`. Add inline comments to call sites explaining the SSRF risk to prevent accidental reversion to the insecure global `fetch`.
