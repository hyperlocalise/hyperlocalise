...
## 2027-07-25 - SIMD-accelerated string scanning and zero-allocation parsing in React-Intl message extraction
**Learning:** In React-Intl message extraction from TS/TSX files: 1) String literals are predominantly plain text without escapes or HTML entities, so fast-paths using `strings.IndexAny` and checking `strings.IndexByte(s, '\\') < 0` / `strings.IndexByte(s, '&') < 0` bypass `strings.Builder` and `html.UnescapeString` allocations entirely; 2) Extracting fixed message descriptor properties directly into local variables bypasses `map[string]string` allocations per descriptor; 3) Replacing `unicode.IsSpace` with ASCII byte checks eliminates UTF-8 rune decoding in whitespace scanning loops.
**Action:** Fast-path string literal scanning with SIMD `strings.IndexAny`, skip unescaping calls when escape signals are absent, and extract object properties into direct variables in AST extraction hot-paths.
