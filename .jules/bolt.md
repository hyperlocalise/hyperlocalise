...
## 2027-07-15 - Direct line streaming and zero-allocation sub-slicing in Subtitle cue encoding
**Learning:** In subtitle marshaling (.srt/.vtt), `encodeSubtitleCueText` created severe allocation spikes due to `strings.ReplaceAll`, `strings.Split`, intermediate `kept` `[]string` slice allocations, and `strings.Join`. Streaming lines directly into `strings.Builder` via an IndexByte loop, trimming with zero-allocation sub-slicing (`s[:end]`), and checking blankness via an ASCII byte loop (`isBlankSubtitleLine`) completely eliminates intermediate slice allocations.
**Action:** Stream line-by-line transformations directly into destination `strings.Builder` instances and use zero-allocation sub-slicing and byte scanners to avoid `strings.Split` and `strings.Join` overhead in document render loops.

## 2027-07-20 - Lazy slice allocation for extra key collection in CSV marshaling
**Learning:** In template-based document marshaling (such as CSV), tracking matched keys in a `seen` map allows identifying new/extra keys that need appending. Using `len(seen) < len(values)` as a guard condition is unsafe because `seen` may contain keys from the template that were not in `values` (e.g. untranslated or removed keys), leading to silent omission of new keys. Instead, lazy allocation (`if keys == nil { keys = make(...) }`) upon encountering the first un-matched key safely avoids heap allocation when all keys in `values` were matched, while preserving 100% correctness under all template/value combinations.
**Action:** Use lazy slice allocation (`if keys == nil { keys = make(...) }`) when collecting un-matched keys during template rendering instead of comparing map lengths.

## 2027-07-25 - Candidate Pre-computation in Glob Matching and SIMD IndexAny Token Skipping
**Learning:** In file resolution pipelines with ignore patterns, re-computing candidate path variants (e.g. `filepath.Clean`, `filepath.Rel`, `filepath.Base`) inside nested loops for every ignore pattern creates massive heap allocation and CPU overhead ($O(N_{\text{files}} \times N_{\text{ignores}})$). Pre-computing candidate paths once per file reduces allocations significantly. Additionally, skipping non-signal characters using `strings.IndexAny` in stateful source code scanners speeds up parsing loop bounds using SIMD instructions.
**Action:** Pre-compute candidate paths once per file before matching against multiple glob patterns, and leverage `strings.IndexAny` for fast SIMD scanning across non-signal tokens in source text parsers.

## 2027-08-01 - Lazy line number calculation in streaming XML token parsers
**Learning:** In streaming XML parsers (e.g. `encoding/xml.Decoder`), maintaining an eager line counter by counting newlines (`strings.Count`) on every returned token adds substantial CPU overhead to the main parsing loop, even though line numbers are only needed for error messages or occasional structural element tracking. Deferred lazy line calculation (`1 + strings.Count(text[:offset], "\n")`) called strictly when errors occur or element positions are recorded completely removes newline counting overhead from normal execution paths.
**Action:** Compute line numbers lazily at the point of error formatting or key line initialization instead of updating line counts on every token loop iteration in document parsers.

## 2027-09-08 - Eliminating Secondary JSON Decoding in Struct Trees and Small-Slice Linear Intersection Fast Paths
**Learning:** In Go JSON tree evaluation (such as experiment criterion evaluation), using `json.RawMessage` for leaf node values forces secondary `json.Unmarshal` calls for every leaf during evaluation. Changing leaf values to `Value any` lets Go's standard `json.Unmarshal` decode primitive values (strings, numbers, slices) in a single pass during initial tree construction, completely eliminating secondary unmarshaling during evaluation. Furthermore, for small slice intersection checks (`len(left) * len(right) <= 16`), nested linear loops bypass heap allocation of intermediate `map[string]struct{}` hash sets.
**Action:** Use `any` instead of `json.RawMessage` for leaf values in JSON AST/tree structures where values are evaluated repeatedly, and use linear loops for small slice intersections to avoid map allocations.

## 2027-09-09 - Stack-allocated buffer formatting and allocation-free tag analysis in scoring evaluator
**Learning:** In evaluation scoring and token extraction hot paths: 1) using `strings.Builder` with `b.Grow()` for short string keys (like HTML tag tokens or ICU block signatures) allocates both a `[]byte` slice header on the heap and the result string. Using a stack-allocated byte buffer `[128]byte` for formatting strings under 128 bytes takes only 1 heap allocation (for the returned string map key); 2) running `strings.ToLower` unconditionally on input tags creates intermediate string allocations. Using `strings.EqualFold` on trimmed string slices avoids allocations when matching tag prefixes.
**Action:** Format small structured map keys (<= 128 bytes) into stack-allocated `[128]byte` buffers instead of `strings.Builder`, and use `strings.EqualFold` for case-insensitive tag prefix matching to eliminate heap allocations.

## 2027-09-10 - Zero-allocation comment slicing and direct key string building in JSONC parser
**Learning:** In comment extraction pipelines for translation files (like `parseJSONCKeyComments`), converting comment lines to heap-allocated `string` objects as soon as they are scanned creates significant garbage collection pressure. Slicing comments as `[]byte` directly from the raw content buffer delays `string` allocations until storing comments into the final context map. Reusing the `pendingComments` slice header (`pendingComments[:0]`) prevents slice re-allocations across keys. Furthermore, returning `keyBytes []byte` from key line scanners and using `strings.Builder` (`b.Write(keyBytes)`) when appending stack prefixes avoids intermediate key string allocations.
**Action:** Slice comment lines and keys as `[]byte` views into the raw input buffer during sequential parsing, and defer `string` conversion until final map insertion or string builder assembly.

## 2027-09-16 - Optimizing Segment Validation Slice Allocations and Extension Matching
**Learning:** In CAT tool segment validation flows, 1) `KindForSourcePath` calling `strings.ToLower(filepath.Ext(path))` creates unnecessary string allocations on every validation call; matching common lowercase file extensions directly on `filepath.Ext(path)` avoids string allocation; 2) initializing `checks` slice capacity in `ValidateSegment` to 1 instead of 2 saves memory overhead (112 bytes vs 224 bytes allocated); 3) refactoring `qaChecks` to append directly into the destination slice avoids allocating intermediate slices.
**Action:** Use direct extension matching on `filepath.Ext` to bypass `strings.ToLower`, size initial result slices accurately for the common single-item path, and append directly into existing slices to avoid secondary slice allocations.

## 2027-09-18 - Optimizing Dictionary Word Normalization, Trimming, and Line Parsing
**Learning:** In spellcheck dictionary import pipelines (`apps/go-svc`): 1) `trimDictionaryInput` using closures and `strings.ContainsRune` generates unnecessary function allocations and multi-rune scans per character; replacing it with a top-level function and ASCII non-whitespace boundary fast-path bypasses trimming entirely for standard words; 2) `cases.Lower(language.English)` instantiates a new Caser on every word unless cached in a package-level variable; 3) `norm.NFC.String` is an expensive no-op for plain ASCII strings, which can be bypassed via a simple `isASCII` byte check; 4) `parseDictionaryWords` using `strings.Split` allocates a large `[]string` slice for file lines, which can be replaced with an `IndexByte` streaming loop and pre-allocated slices/maps using line count hints (`strings.Count(content, "\n") + 1`).
**Action:** Use package-level Caser instances, bypass Unicode normalization on plain ASCII strings, stream line processing with `IndexByte`, pre-allocate slices/maps with line count hints, and use top-level trim functions with ASCII fast-paths.

## 2027-09-21 - Linear deduplication scan and canonical length fast-path in parseFindingIDs
**Learning:** Instantiating `make(map[uuid.UUID]struct{}, len(raw))` allocates a map on the heap and computes hashes for each UUID. When input slices have bounded small maximum lengths (such as `n <= 100`), performing a linear scan over the result slice avoids map allocations entirely while remaining cache-friendly and faster than map insertion/lookup. Additionally, checking `len(item) == 36` serves as a zero-allocation fast-path to bypass `strings.TrimSpace` for standard canonical UUID string representations.
**Action:** Use linear deduplication scans for slice processing when upper bounds on slice lengths are small ($N \le 100$), and bypass string trimming for fixed-length canonical formats.

## 2027-09-22 - Single-Pass Whitespace Collapsing and ASCII Fast-Path for String Normalization
**Learning:** In string normalization routines (such as translation memory source text normalization), using `regexp.MustCompile(`\s+`)` and `strings.TrimSpace` creates significant regex execution and intermediate string allocation overhead. Replacing `regexp` with a single-pass streaming loop using `strings.Builder` and a `pendingSpace` boolean flag trims, collapses contiguous whitespace, and lowercases in one pass. Additionally, adding an ASCII fast-path (`isASCII`) bypasses Unicode NFKC normalization (`norm.NFKC.String`), and checking if an ASCII string is already normalized (`isNormalizedASCII`) returns it directly with 0 heap allocations.
**Action:** Replace `regexp` whitespace replacements with single-pass `pendingSpace` state machine loops, bypass Unicode normalization for plain ASCII strings, and return pre-normalized strings directly to achieve zero allocations.
