...
## 2027-05-18 - Optimizing Fluent Parser with full line capacity hinting and zero-allocation indents
**Learning:** For Mozilla Fluent FTML localization files, sequential parsers can be optimized by adjusting entries slice capacity hints to match full lines to avoid any dynamic slice growth. Furthermore, pre-allocating strings.Builder via Grow and adding early returns for empty comment arrays prevents any helper builder allocations on the hot path. Finally, implementing manual string-checking fast paths for the most common indent widths (such as 0-width and 4-space indentation) in continuation indent helpers avoids expensive string concatenations and allocations entirely.
**Action:** Initialize slice capacities to len(lines) instead of len(lines)/2 when entries density is high, add early returns to helper format builders, and bypass slice/string concatenations using manual indentation fast paths.
