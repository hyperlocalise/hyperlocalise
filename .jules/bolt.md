...
## 2027-05-01 - Single-map token similarity counting and F1 score optimization
**Learning:** In text similarity metrics (like token-based F1 scoring), constructing two independent frequency maps (`rCount` and `cCount`) to compute their multiset intersection introduces redundant heap allocations and map-rehashing overhead. Pre-allocating a single frequency map sized to the reference token list, and then decrementing those counts as matching tokens from the candidate string are processed, mathematically produces the exact same F1 score while reducing garbage collection pressure and avoiding a second map allocation entirely.
**Action:** Always pre-allocate a single token-count map for F1/set-similarity scores instead of allocating multiple intermediate structures.
