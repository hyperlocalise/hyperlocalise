...
## 2027-03-24 - Zero-allocation XML well-formedness scanner for Android resources
**Learning:** Initializing an `xml.Decoder` for every XML-containing segment is highly expensive, especially when translating thousands of segments. Implementing a fast, stack-allocated, zero-allocation scanner can bypass `xml.Decoder` entirely for the common cases of translation fragments (like HTML tags `<b>`, self-closing `<br/>`, etc.). However, XML tag/attribute names can legally contain Unicode (non-ASCII) characters. Restricting the scanner to ASCII alphanumeric tag names can cause false well-formedness rejections for valid non-ASCII tags unless a fallback is explicitly triggered. Detecting any byte `>= 0x80` while scanning tags allows us to safely and selectively fall back to the slow `xml.Decoder` while preserving the fast path for standard ASCII tags.
**Action:** Pre-scan XML/HTML tags with a zero-allocation state machine, and trigger a fallback (returning certain=false) if non-ASCII bytes are encountered inside tag boundaries.
