...
## 2027-03-10 - Sorted and unique fast-paths and stack backing arrays in ICU parser
**Learning:** Functions that compare placeholder sets (`SamePlaceholderSet`) or deduplicate lists (`uniqueStrings`) often allocate heap memory for cloning and sorting slices. In the main application path, the slices (e.g. `inv.Placeholders` from `ParseInvariant`) are already sorted and unique. A fast, $O(N)$ zero-allocation check can bypass copying, sorting, and deduplication entirely. Additionally, for small intermediate slices like plural options, using a stack-allocated backing array avoids heap allocation — but only if escape analysis keeps it on the stack through `append` and `slices.SortFunc`. Confirm with `go build -gcflags="-m"` before documenting as zero-allocation.
**Action:** Implemented `isSortedAndUnique` check to enable zero-allocation (`0 B/op`, `0 allocs/op`) comparisons in `SamePlaceholderSet`, yielding a 6x speedup. Added a stack backing array of size 8 for intermediate signatures in `sortedPluralOptionSignatures` (verified: `localSigs` does not escape). Documented that `uniqueStrings` may return its input slice on the fast path.
