...
**Action:** Implemented `isSortedAndUnique` check to enable zero-allocation (`0 B/op`, `0 allocs/op`) comparisons in `SamePlaceholderSet`, yielding a 6x speedup. Added a stack backing array of size 8 for intermediate signatures in `sortedPluralOptionSignatures`.
