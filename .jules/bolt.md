...
## 2027-09-11 - Pre-screening source strings for ICU token signal characters
**Learning:** In segment validation (`validateICUInvariantWithTokens`), calling `icuparser.ParseInvariant` unconditionally on plain text strings forces AST parsing and element collection for strings that contain no ICU placeholders or plural blocks. Pre-screening the `source` string with `strings.ContainsAny(source, "{#")` immediately identifies plain text strings, bypassing space trimming and expensive ICU AST construction.
**Action:** Pre-screen source strings for signal characters (`{` and `#`) before invoking AST parsers in validation hot paths.
